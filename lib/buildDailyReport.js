...
export function buildDailyReport({ sales, receipts, lodgements, consumption, nozzles, tanks, banks, startDate, endDate }) {
  ...
  const dateReports = rangeResult.dates.map(({ date, entries: helperEntries, hasEntry, entryCount }) => {
    ...
    // ── Consumption (from dedicated consumption entries) ──
    const dayConsumption = consumptionByDate[date] || { entries: [], byFuelType: {}, totalConsumed: 0, totalPourBack: 0 }

    // ── Consumption comparison: consumption entries vs daily sales nozzle readings ──
    // Daily sales nozzle readings totals (from dayFuelTotals above)
    // Consumption entries totals (from helper)
    const consumptionComparison = {}
    for (const ft of fuelTypes) {
      const nozzleConsumed = dayFuelTotals[ft].consumed
      const nozzlePourBack = dayFuelTotals[ft].pourBack
      const entryConsumed = dayConsumption.byFuelType[ft]?.consumed || 0
      const entryPourBack = dayConsumption.byFuelType[ft]?.pourBack || 0
      consumptionComparison[ft] = {
        nozzleConsumed,
        nozzlePourBack,
        entryConsumed,
        entryPourBack,
        consumedMatch: nozzleConsumed === entryConsumed,
        pourBackMatch: nozzlePourBack === entryPourBack,
      }
    }
