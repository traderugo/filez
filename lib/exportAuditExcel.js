...
function writeSalesCash(wb, report, stationName, startDate, endDate, refs) {
  ...
  for (let ftIdx = 0; ftIdx < fuelTypes.length; ftIdx++) {
    ...
    // 2 spacer rows before next fuel type — outer edges only (L:medium on B, R:medium on G)
    if (ftIdx < fuelTypes.length - 1) {
      for (let s = 0; s < 2; s++) {
        ws.getRow(row).getCell(2).border = { left: { style: 'medium' } }
        ws.getRow(row).getCell(7).border = { right: { style: 'medium' } }
        row++
      }
    }
  }
