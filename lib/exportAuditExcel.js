...
function writeGuideline(wb) {
  ...
  const labelStyle = { font: FONT_BOLD, border: BORDER_MED_BOX, alignment: { horizontal: 'left', vertical: 'top' } }
  const contentStyle = { font: FONT, border: BORDER_MED_BOX, alignment: { horizontal: 'left', vertical: 'top', wrapText: true } }
...
function writeSalesCash(wb, report, stationName, startDate, endDate, refs) {
  ...
  for (let ftIdx = 0; ftIdx < fuelTypes.length; ftIdx++) {
    ...
    // Extra blank pour back rows (template has ~5 rows allocated)
    const pourBackAllocated = 5
    ...
    // Extra blank consumption rows (template has ~12 rows allocated)
    const consumeAllocated = 12
