...
function buildSalesCashPosition({ sales, lodgements, consumption, nozzles, banks, fuelTypes, customerMap, startDate, endDate }) {
  ...
  for (const dayData of consumptionResult.dates) {
    for (const e of dayData.entries) {
      ...
      if (e.isPourBack) {
        consumptionByFuel[ft].pourBack.push({ name, qty, customerId: e.customerId })
        consumptionByFuel[ft].totalPourBackQty += qty
      } else {
        consumptionByFuel[ft].consumed.push({ name, qty, customerId: e.customerId })
        consumptionByFuel[ft].totalConsumedQty += qty
      }
    }
  }

  // Aggregate consumption/pour back by customer name
  for (const ft of fuelTypes) {
    consumptionByFuel[ft].consumed = aggregateByName(consumptionByFuel[ft].consumed)
    consumptionByFuel[ft].pourBack = aggregateByName(consumptionByFuel[ft].pourBack)
  }
  ...
  for (const ft of fuelTypes) {
    ...
    // Pour back amount: use the price from the last row (simplification — could be multi-price)
    const lastPrice = rows.length > 0 ? rows[rows.length - 1].price : 0
    const totalPourBackAmt = totalPourBackQty * lastPrice
    consumptionByFuel[ft].totalPourBackAmt = totalPourBackAmt
    ...
    const totalConsumedAmt = totalConsumedQty * lastPrice
    consumptionByFuel[ft].totalConsumedAmt = totalConsumedAmt
...
/**
 * Aggregate an array of { name, qty } objects, summing qty by name.
 */
