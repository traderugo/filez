...
export function useRemoteChanges(orgId) {
  ...
  const poll = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/entries/poll?org_id=${orgId}&since=${encodeURIComponent(lastPollRef.current)}`, { cache: 'no-store' })
      if (!res.ok) return
      ...
      for (const [localTable, records] of Object.entries(changes)) {
        for (const record of records) {
          try {
            // If the server record is soft-deleted, remove it locally
            if (record.deleted_at) {
              await db[localTable].delete(record.id)
              continue
            }
