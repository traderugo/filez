...
function buildFuelMeterRows(fuelType, ftNozzles, pumpIds, entries, openings, initPrice) {
  ...
  function emitRow(price) {
    ...
    // Skip zero-dispensed periods (e.g., COB entries that only carry a price marker)
    if (totalDispensed === 0) return
  ...
  // Merge consecutive rows at the same price.
  // This happens when a zero-dispensed period (e.g., COB entry) was skipped
  // between two periods at the same price, leaving them adjacent.
