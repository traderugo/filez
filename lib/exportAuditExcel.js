...
function writeSalesCash(wb, report, stationName, startDate, endDate, refs, logoId) {
  ...
  for (let ftIdx = 0; ftIdx < fuelTypes.length; ftIdx++) {
    ...
    // Fix formulas: A should use actual sales (dispensed - consumption), not raw dispensed.
    // Subtract consumption from A so amount = (dispensed - consumption) × price.
    const aStyleE = { font: FONT_BOLD, fill: YELLOW_FILL, border: BORDER_THIN, alignment: { horizontal: 'center' } }
    formulaYellow(ws, ftRefs.totalARow, 5, `SUM(E${ftRefs.firstDataRow}:E${ftRefs.lastDataRow})-SUM(E${ftRefs.consumeStart}:E${ftRefs.consumeEnd})`, aStyleE)
    formulaYellow(ws, ftRefs.totalARow, 7, `SUM(G${ftRefs.firstDataRow}:G${ftRefs.lastDataRow})-SUM(G${ftRefs.consumeStart}:G${ftRefs.consumeEnd})`, aStyleE)

    // Zero D (consumption already factored into A) to avoid double-counting in E=C-D.
    const dStyleE = { font: FONT_BOLD, fill: YELLOW_FILL, border: BORDER_THIN, alignment: { horizontal: 'center' } }
    formulaYellow(ws, ftRefs.totalDRow, 5, '0', dStyleE)
    formulaYellow(ws, ftRefs.totalDRow, 7, '0', dStyleE)
