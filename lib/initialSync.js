...
export async function initialSync(orgId, { force = false } = {}) {
  ...
  for (const { api, table, key } of entryTypes) {
    try {
      ...
      // Debug: log raw entry dates vs normalized dates
      if (allEntries.length && api === 'daily-sales') {
        allEntries.forEach((e, i) => {
          const norm = normalizeEntry(table, e, orgId)
          console.log(`[initialSync] daily-sales[${i}]: raw entry_date=${e.entry_date}, normalized entryDate=${norm.entryDate}, id=${e.id}`)
        })
      }
