...
export default function ProductReceiptListPage() {
  ...
  const limit = 10
  const { subscribed, loading: subLoading } = useSubscription(orgId, 'fuel-operations')
    ...
    <div className="max-w-3xl mx-auto px-4 sm:px-8 py-8">
      ...
      <div className="flex items-center justify-end mb-6">
        {(subscribed || subLoading) ? (
          <Link href={`/dashboard/entries/product-receipt?${qs}`} className="flex items-center gap-1 text-sm bg-primary-500 text-white px-4 py-2 font-medium hover:bg-primary-600">
              ...
              <div key={group.date} className={`py-3 px-2 flex items-center gap-3 ${hasUnsynced ? 'bg-green-50 dark:bg-green-950/40' : ''}`}>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-content">
                    ...
                    <span className="ml-2 text-xs px-1.5 py-0.5 rounded-full bg-subtle text-content-muted font-medium">{group.entries.length} {group.entries.length === 1 ? 'receipt' : 'receipts'}</span>
                  </p>
                  <p className="text-xs text-content-muted">
                    {group.entries.map((e, i) => (
                      <span key={e.id}>
                        ...
                        {e.driverName && <span className="text-content-faint"> ({e.driverName})</span>}
                      </span>
                    ))}
                  </p>
                </div>
                <Link href={`/dashboard/entries/product-receipt?${qs}&edit_date=${group.date}`} className="flex items-center gap-1 text-xs font-medium text-primary-600 border border-primary-500/40 px-3 py-1.5 rounded hover:bg-primary-50">
