 ...
 *   - Audit Report is its own group rather than store's "Activity". Store's Activity is a
 *     who-did-what log; this is a multi-sheet financial pack (sales/cash position, lodgement
 *     sheet, stock position, consumption, calculator). Folding it elsewhere would misname it.
 */
const REPORT_GROUPS = [
  { title: 'Sales', keys: ['report-summary', 'report-sales-overview', 'report-daily-sales', 'report-sales-operation', 'report-analytics'] },
  { title: 'Stock', keys: ['report-inventory-log', 'report-product-received', 'report-lube'] },
  { title: 'Cash & Accounts', keys: ['report-account-ledger', 'imprest'] },
  { title: 'Audit', keys: ['report-audit'] },
]
...
export function REPORT_SECTIONS(stationId) {
  ...
  const sections = REPORT_GROUPS
    .map((g) => ({ title: g.title, items: g.keys.map((k) => byKey.get(k)).filter(Boolean) }))
