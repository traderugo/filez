  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-8 py-8">
      <div className="flex items-center justify-end mb-6">
        <Link href={`/dashboard/entries/customer-payments?${qs}`} className={`flex items-center gap-1 text-sm px-4 py-2 font-medium ${BTN_PRIMARY}`}>
          ...
          <div className="divide-y divide-line">
            {pageGroups.map((group) => {
              ...
              <div key={group.date} className={`py-3 px-2 flex items-center gap-3 ${hasUnsynced ? 'bg-green-50 dark:bg-green-950/40' : ''}`}>
                ...
                <Link href={`/dashboard/entries/customer-payments?${qs}&edit_date=${group.date}`} className={`flex items-center gap-1 text-xs font-medium px-3 py-1.5 ${BTN_FRAMED}`}>
