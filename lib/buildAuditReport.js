...
function buildStockPosition({ sales, receipts, nozzles, tanks, fuelTypes, startDate, endDate }) {
  ...
  let _prevCobEntryDate = null
  for (let i = sorted.length - 1; i >= 0; i--) {
    if (sorted[i].entryDate < startDate) {
      prevCobTankReadings = sorted[i].tankReadings || []
      _prevCobEntryDate = sorted[i].entryDate
      break
    }
  }

  // DEBUG: remove after diagnosing May 2 opening=0 in Stock Position
  // eslint-disable-next-line no-console
  console.log('[DEBUG stock-position]', { startDate, prevCobEntryDate: _prevCobEntryDate, prevCobTankReadings, tankIds: tanks.map(t => t.id) })
