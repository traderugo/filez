...
function buildFuelMeterRows(fuelType, ftNozzles, pumpIds, entries, openings, initPrice) {
  ...
  let hasClosings = false
  for (const entry of entries) {
    if (entry.price !== currentPrice) {
      // Only emit if at least one closing has been read; otherwise just update
      // the price to avoid a spurious zero-dispensed row when the price changed
      // between prevEntry and the first range entry.
      if (hasClosings) emitRow(currentPrice)
      currentPrice = entry.price
    }

    for (const pid of pumpIds) {
      if (entry.closings[pid] != null) { lastClosing[pid] = entry.closings[pid]; hasClosings = true }
    }
  }
