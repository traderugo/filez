...
// Border styles
// DATA = reference pattern for data rows: bottom+left+right (no top)
const BORDER_DATA = {
  bottom: { style: 'thin' }, left: { style: 'thin' }, right: { style: 'thin' },
}
const BORDER_DATA_L = { ...BORDER_DATA, left: { style: 'medium' } }
const BORDER_DATA_R = { ...BORDER_DATA, right: { style: 'medium' } }
// THIN = all 4 sides thin (used only where reference explicitly has top too)
const BORDER_THIN = {
...
function dataStyle(align = 'center', font) {
  return { font: font || FONT, border: BORDER_DATA, alignment: { horizontal: align } }
}
function dataStyleYellow(align = 'center', font) {
  return { font: font || FONT, fill: YELLOW_FILL, border: BORDER_DATA, alignment: { horizontal: align } }
}
function boldStyle(align = 'center', font) {
  return { font: font || FONT_BOLD, border: BORDER_DATA, alignment: { horizontal: align } }
}
...
function fillGrid(ws, startRow, endRow, firstCol, lastCol, font) {
  ...
  for (let r = startRow; r <= endRow; r++) {
    for (let c = firstCol; c <= lastCol; c++) {
      ...
      if (!hasBorder) {
        // Empty cell — apply data border + font
...
function writeSalesCash(wb, report, stationName, startDate, endDate, refs) {
  const ws = wb.addWorksheet('2. Sales>>Cash Position')
  ...
  for (let ftIdx = 0; ftIdx < fuelTypes.length; ftIdx++) {
    ...
    textCell(ws, row, 5, '    ', hdrStyle(FONT_BOLD))  // Dispensed header (blank per ref — labeled in yellow col)
    ...
    // Pump meter rows — grouped by price
    for (let ri = 0; ri < summary.rows.length; ri++) {
      ...
      for (const pump of period.pumps) {
        ...
        // E = D - C (dispensed) — yellow formula
        const eRef = `D${row}-C${row}`
        formulaYellow(ws, row, 5, eRef, dataStyleYellow(), NUM_FMT)

        numCell(ws, row, 6, period.price, dataStyle())

        // G = E * F (amount) — yellow formula
        const gRef = `E${row}*F${row}`
        formulaYellow(ws, row, 7, gRef, dataStyleYellow())
        row++
      }
    }
    ...
    // Extra blank pour back rows (min 5 rows allocated, grows dynamically)
    const pourBackAllocated = Math.max(5, pourBackGroups.length)
    const pourBackWritten = Math.max(pourBackGroups.length, 1)
    for (let i = pourBackWritten; i < pourBackAllocated; i++) {
      formulaYellow(ws, row, 7, `E${row}*F${row}`, dataStyleYellow())
      row++
    }
    ...
    // Extra blank consumption rows (min 12 rows allocated, grows dynamically)
    const consumeAllocated = Math.max(12, consumedGroups.length)
    const consumeWritten = consumedGroups.length
    for (let i = consumeWritten; i < consumeAllocated; i++) {
      formulaYellow(ws, row, 7, `E${row}*F${row}`, dataStyleYellow())
      row++
    }
  ...
  textCell(ws, row, 2, `SALES/LODGEMENT RECONCIALATION ${fmtDateDisplay(startDate)}-${fmtDateDisplay(endDate)}`, { font: FONT_BOLD, border: BORDER_THIN, alignment: { horizontal: 'left' } })
  row++

  // Blank spacer
  row++

  // Expected Sales from Products — sum of all fuel types' expected rows
  ...
  textCell(ws, row, 2, 'Expected Sales from Products for the Period ', { font: FONT_BOLD, border: BORDER_THIN, alignment: { horizontal: 'left' } })
  formulaYellow(ws, row, 7, expectedFormula, { font: FONT_BOLD, fill: YELLOW_FILL, border: BORDER_THIN, alignment: { horizontal: 'center' } })
  row++

  // Blank spacer
  row++

  // Less expenses — references Expenses sheet
  refs.expensesRow = row
  textCell(ws, row, 2, 'Less expenses for the Period(Approved)', { font: FONT_BOLD, border: BORDER_THIN, alignment: { horizontal: 'left' } })
  formulaYellow(ws, row, 7, "'9.Expenses for the Month'!E38", { font: FONT_BOLD, fill: YELLOW_FILL, border: BORDER_THIN, alignment: { horizontal: 'center' } })
  row++
  ...
  textCell(ws, row, 2, 'Net Expected Sales for the Period', { font: FONT_BOLD, border: BORDER_THIN, alignment: { horizontal: 'left' } })
  formulaYellow(ws, row, 7, `G${refs.expectedSalesRow}-G${refs.expensesRow}`, { font: FONT_BOLD, fill: YELLOW_FILL, border: BORDER_THIN, alignment: { horizontal: 'center' } })
  row++

  // Blank spacer
  row++

  // Total Lodgement — references Lodgement sheet
  refs.totalLodgementRow = row
  textCell(ws, row, 2, 'Total lodgement for the Period', { font: FONT_BOLD, border: BORDER_THIN, alignment: { horizontal: 'left' } })
  formulaYellow(ws, row, 7, `'4.Lodgement Sheet'!T${refs.lodgementTotalRow || 172}`, { font: FONT_BOLD, fill: YELLOW_FILL, border: BORDER_THIN, alignment: { horizontal: 'center' } })
  row++

  // Blank spacer
  row++

  // Total POS — references Lodgement sheet
  textCell(ws, row, 2, 'Total POS for the Period', { font: FONT_BOLD, border: BORDER_THIN, alignment: { horizontal: 'left' } })
  formulaYellow(ws, row, 7, `'4.Lodgement Sheet'!Q${refs.lodgementTotalRow || 172}`, { font: FONT_BOLD, fill: YELLOW_FILL, border: BORDER_THIN, alignment: { horizontal: 'center' } })
  ...
  // Opening Balance of Debit/Credit customers — references Customers' Ledger
  textCell(ws, row, 2, 'Opening Balance of Debit/Credit customers', { font: FONT_BOLD, border: BORDER_THIN, alignment: { horizontal: 'left' } })
  formulaYellow(ws, row, 7, "'Customers'' Ledger'!C43", { font: FONT_BOLD, fill: YELLOW_FILL, border: BORDER_THIN, alignment: { horizontal: 'center' } })
  ...
  textCell(ws, row, 2, 'Closing Balance of Debit/Credit customers', { font: FONT_BOLD, border: BORDER_THIN, alignment: { horizontal: 'left' } })
  formulaYellow(ws, row, 7, "'Customers'' Ledger'!J43", { font: FONT_BOLD, fill: YELLOW_FILL, border: BORDER_THIN, alignment: { horizontal: 'center' } })
  ...
  // Overage/Shortage
  textCell(ws, row, 2, 'Overage/Shortage', { font: FONT_BOLD, border: BORDER_THIN, alignment: { horizontal: 'left' } })
  formulaYellow(ws, row, 7, `(G${refs.totalLodgementRow}+G${refs.totalPOSRow}+G${refs.openingBalRow}-G${refs.closingBalRow})-G${refs.netExpectedRow}`, { font: FONT_BOLD, fill: YELLOW_FILL, border: BORDER_THIN, alignment: { horizontal: 'center' } })

  // Fill all empty cells in the data grid with borders
  fillGrid(ws, 7, row, 2, 7, FONT)
}

// ─── 3. Stock Position ───────────────────────────────────────

function writeStockPosition(wb, report, stationName, startDate, endDate, refs) {
  const ws = wb.addWorksheet('3.Stock Position')
  ...
  for (const ft of fuelTypes) {
    ...
    // Section header: yellow fill
    textCell(ws, row, 3, `${ft} STOCK POSITION`, { font: FONT_12_BOLD, fill: YELLOW_FILL, border: BORDER_THIN, alignment: { horizontal: 'left' } })
    textCell(ws, row, 4, "Per Manager's Computation", { font: FONT_12_BOLD, fill: YELLOW_FILL, border: BORDER_THIN, alignment: { horizontal: 'left' } })
    ws.getRow(row).height = 25.5
    row++

    // OPENING STOCK — formula referencing Record of Stock Position
    textCell(ws, row, 3, `OPENING STOCK (${fmtDateDisplay(startDate)})`, { font: FONT_BOLD, border: BORDER_THIN, alignment: { horizontal: 'left' } })
    if (fMap.openCol) {
      formulaCell(ws, row, 4, `'10.Record of Stock Position'!${fMap.openCol}8`, dataStyle())
    } else {
      numCell(ws, row, 4, t.opening, dataStyle())
    }
    row++
    ...
    // SUPPLIES DURING THE PERIOD — formula referencing Product Received
    textCell(ws, row, 3, 'SUPPLIES DURING THE PERIOD ', { font: FONT_BOLD, border: BORDER_THIN, alignment: { horizontal: 'left' } })
    if (fMap.prodActCol) {
      formulaCell(ws, row, 4, `'8.Product Received'!${fMap.prodActCol}52`, dataStyle())
    } else {
      numCell(ws, row, 4, t.supply, dataStyle())
    }
    row++
    ...
    textCell(ws, row, 3, 'STOCK AVAILABLE FOR SALE', { font: FONT_BOLD, border: BORDER_THIN, alignment: { horizontal: 'left' } })
    formulaCell(ws, row, 4, `+D${openRow}+D${supplyRow}`, dataStyle())
    ...
    // CLOSING STOCK — formula referencing Record of Stock Position
    textCell(ws, row, 3, `CLOSING STOCK (${fmtDateDisplay(endDate)})`, { font: FONT_BOLD, border: BORDER_THIN, alignment: { horizontal: 'left' } })
    if (fMap.recCol) {
      formulaCell(ws, row, 4, `'10.Record of Stock Position'!${fMap.recCol}40`, { font: FONT_BOLD, border: BORDER_THIN, alignment: { horizontal: 'center' } })
    } else {
      numCell(ws, row, 4, t.closing, { font: FONT_BOLD, border: BORDER_THIN, alignment: { horizontal: 'center' } })
    }
    ...
    textCell(ws, row, 3, 'QUANTITY SOLD', { font: FONT_BOLD, border: BORDER_THIN, alignment: { horizontal: 'left' } })
    formulaCell(ws, row, 4, `D${stockAvailRow}-D${closingRow}`, { font: FONT_BOLD, border: BORDER_THIN, alignment: { horizontal: 'center' } })
    ...
    // QUANTITY DISPENSED — formula referencing Sales sheet
    textCell(ws, row, 3, 'QUANTITY DISPENSED ', { font: FONT_BOLD, border: BORDER_THIN, alignment: { horizontal: 'left' } })
    const salesNetRef = refs.sales[ft]?.netRow
    if (salesNetRef) {
      formulaCell(ws, row, 4, `'2. Sales>>Cash Position'!E${salesNetRef}`, { font: FONT_BOLD, border: BORDER_THIN, alignment: { horizontal: 'center' } })
    } else {
      numCell(ws, row, 4, t.dispensed, { font: FONT_BOLD, border: BORDER_THIN, alignment: { horizontal: 'center' } })
    }
    const dispRow = row
    row++
    ...
    // OVERAGE/(SHORTAGE) = DISPENSED - SOLD
    textCell(ws, row, 3, 'OVERAGE/(SHORTAGE)', { font: FONT_BOLD, border: BORDER_THIN, alignment: { horizontal: 'left' } })
    formulaCell(ws, row, 4, `D${dispRow}-D${qtySoldRow}`, { font: FONT_BOLD, border: BORDER_THIN, alignment: { horizontal: 'center' } })
    row++

    // Spacer rows
    row += 3

    // EXPECTED LITRES — formula referencing Product Received
    textCell(ws, row, 3, 'EXPECTED LITRES', { font: FONT_BOLD, border: BORDER_THIN, alignment: { horizontal: 'left' } })
    if (fMap.prodExpCol) {
      formulaCell(ws, row, 4, `'8.Product Received'!${fMap.prodExpCol}52`, dataStyle())
    } else {
      numCell(ws, row, 4, t.expectedLitres, dataStyle())
    }
    ...
    textCell(ws, row, 3, 'ACTUAL LITRES RECEIVED', { font: FONT_BOLD, border: BORDER_THIN, alignment: { horizontal: 'left' } })
    if (fMap.prodActCol) {
      formulaCell(ws, row, 4, `'8.Product Received'!${fMap.prodActCol}52`, dataStyle())
    } else {
      numCell(ws, row, 4, t.actualLitresReceived, dataStyle())
    }
    ...
    // OVERAGE/(SHORTAGE) - TRUCK DRIVER
    textCell(ws, row, 3, 'OVERAGE/(SHORTAGE) - TRUCK DRIVER', { font: FONT_BOLD, border: BORDER_THIN, alignment: { horizontal: 'left' } })
    formulaCell(ws, row, 4, `D${actLitresRow}-D${expLitresRow}`, { font: FONT_BOLD, border: BORDER_THIN, alignment: { horizontal: 'center' } })
    row++
  ...
  // Fill empty cells in the grid area with borders (cols C-D = 3-4)
  fillGrid(ws, 6, row - 1, 3, 4, FONT)
}
...
function writeLodgement(wb, report, stationName, startDate, endDate, refs) {
  ...
  const bankCols = Math.max(displayBanks.length, 7) // At least 7 bank columns like reference
  for (let i = 0; i < bankCols; i++) {
    ws.getColumn(bankStartCol + i).width = 14.71
  }
  ...
  for (const r of visibleRows) {
    ...
    // C = E + G + Q (Total Sales formula)
    const cFormula = `${colLetter(5)}${row}+${colLetter(7)}${row}+${colLetter(qCol)}${row}`
    formulaCell(ws, row, 3, cFormula, dataStyle('center', FONT_10))
    ...
    // Q = sum of bank columns (POS total formula)
    const bankRange = `${colLetter(bankStartCol)}${row}:${colLetter(bankStartCol + bankCols - 1)}${row}`
    formulaCell(ws, row, qCol, `SUM(${bankRange})`, dataStyle('center', FONT_10))

    // S = E - R (Expected Lodgement formula)
    formulaCell(ws, row, sCol, `${colLetter(5)}${row}-${colLetter(rCol)}${row}`, dataStyle('center', FONT_10))
    ...
    formulaCell(ws, row, 3, `${colLetter(5)}${row}+${colLetter(7)}${row}+${colLetter(qCol)}${row}`, dataStyle('center', FONT_10))
    formulaCell(ws, row, qCol, `SUM(${colLetter(bankStartCol)}${row}:${colLetter(bankStartCol + bankCols - 1)}${row})`, dataStyle('center', FONT_10))
    formulaCell(ws, row, sCol, `${colLetter(5)}${row}-${colLetter(rCol)}${row}`, dataStyle('center', FONT_10))
  ...
  // Fill remaining rows up to 171 with blank formula rows (template has 161 data rows)
...
function writeConsumption(wb, report, startDate, endDate, fuelType, sheetName) {
  ...
  // Column widths — starts at B, variable per customer count
  ws.getColumn(1).width = 9.29  // A (empty)
  ws.getColumn(2).width = 14    // B - Date
  ws.getColumn(3).width = 16.71 // C - Rate

  // Customer columns + Pour back column
  for (let i = 0; i < customers.length; i++) {
    ws.getColumn(4 + i).width = Math.max(customers[i].name.length * 1.2, 12)
  }
  ws.getColumn(4 + customers.length).width = 13.29 // Pour back
  ...
  for (const r of dataRows) {
    dateCell(ws, row, 2, r.date, dataStyle('left', FONT_10))
    numCell(ws, row, 3, r.rate, dataStyle('center', FONT_10))
    for (let i = 0; i < customers.length; i++) {
      numCell(ws, row, 4 + i, r.customerQtys[customers[i].id] || 0, dataStyle('center', FONT_10))
    }
    numCell(ws, row, 4 + customers.length, r.pourBack || 0, dataStyle('center', FONT_10))
    row++
  }
...
function writeRecordOfStock(wb, report, refs) {
  const ws = wb.addWorksheet('10.Record of Stock Position')
  const { stockPosition, fuelTypes } = report
  if (!stockPosition) return
  ...
  for (const ft of fuelTypes) {
    ...
    for (let i = 0; i < visibleRows.length; i++) {
      ...
      if (d.supply) numCell(ws, r, cols.supplyCol, d.supply, dataStyle('center', FONT_10))
...
function writeCustomersLedger(wb, customers) {
  const ws = wb.addWorksheet("Customers' Ledger")
...
function writeLubricants(wb, lubeSales, lubeStock, lubeProducts, stationName, startDate, endDate) {
  ...
  // Fill empty rows up to row 29 (template has 25 product rows)
