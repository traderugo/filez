...
function buildConsumptionReport({ sales, consumption, nozzles, fuelTypes, customerMap, defaultCustomers, startDate, endDate }) {
  ...
  // Group consumption entries by date and fuel type (dedup same customer+fuel+qty+type per date)
  const rawRange = (consumption || []).filter(e => {
    const d = e.entryDate || ''
    return d >= startDate && d <= endDate
  })
  const seen = new Set()
  const rangeConsumption = rawRange.filter(e => {
    const key = `${e.entryDate}_${e.customerId}_${e.fuelType}_${e.quantity}_${!!e.isPourBack}`
