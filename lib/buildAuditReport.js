...
function buildSalesCashPosition({ sales, lodgements, consumption, nozzles, banks, fuelTypes, customerMap, startDate, endDate }) {
  ...
  // Consumption rows B/D: only db.consumption entries (per customer).
  // Nozzle readings consumption is for meter reconciliation only and is
  // already factored into the Lodgement Sheet via the inline meter calculation.
  ...
  // Fallback: if no db.consumption entries exist for a fuel type, use
  // nozzle-level consumption from the daily sales entries instead.
  const pumpFuelMap = {}
  for (const n of nozzles) pumpFuelMap[n.id] = n.fuel_type

  for (const ft of fuelTypes) {
    if (consumptionByFuel[ft].totalConsumedQty > 0 || consumptionByFuel[ft].totalPourBackQty > 0) continue
    let nozzleConsumedQty = 0
    let nozzleConsumedAmt = 0
    let nozzlePourBackQty = 0
    let nozzlePourBackAmt = 0
    for (const e of rangeEntries) {
      const price = Number(e.prices?.[ft] || 0) || getPriceForDate(ft, e.entryDate)
      for (const r of (e.nozzleReadings || [])) {
        if (pumpFuelMap[r.pump_id] !== ft) continue
        const consumed = Number(r.consumption || 0)
        const pourBack = Number(r.pour_back || 0)
        nozzleConsumedQty += consumed
        nozzleConsumedAmt += consumed * price
        nozzlePourBackQty += pourBack
        nozzlePourBackAmt += pourBack * price
      }
    }
    if (nozzleConsumedQty > 0) {
      const avgPrice = Math.round(nozzleConsumedAmt / nozzleConsumedQty)
      consumptionByFuel[ft].consumed.push({ name: 'Meter consumption', qty: nozzleConsumedQty, price: avgPrice, amt: nozzleConsumedAmt, customerId: null })
      consumptionByFuel[ft].totalConsumedQty = nozzleConsumedQty
      consumptionByFuel[ft].totalConsumedAmt = nozzleConsumedAmt
    }
    if (nozzlePourBackQty > 0) {
      const avgPrice = Math.round(nozzlePourBackAmt / nozzlePourBackQty)
      consumptionByFuel[ft].pourBack.push({ name: 'Meter pour back', qty: nozzlePourBackQty, price: avgPrice, amt: nozzlePourBackAmt, customerId: null })
      consumptionByFuel[ft].totalPourBackQty = nozzlePourBackQty
      consumptionByFuel[ft].totalPourBackAmt = nozzlePourBackAmt
    }
  }
...
function buildLodgementSheet({ sales, lodgements, consumption, nozzles, banks, fuelTypes, startDate, endDate }) {
  ...
  const rows = dateStrings.map((date, idx) => {
    ...
    // Step 1: Calculate dispensed amounts and nozzle-level consumption
    let totalSales = 0
    const dayDispensed = {}
    const nozzleConsumed = {}
    const nozzlePourBack = {}
    for (const ft of fuelTypes) { dayDispensed[ft] = 0; nozzleConsumed[ft] = 0; nozzlePourBack[ft] = 0 }
    ...
    for (const entry of dayEntries) {
      ...
      for (const ft of fuelTypes) {
        ...
        for (const n of nozzlesByFuel[ft]) {
          ...
          prevClosing[n.id] = closing
          nozzleConsumed[ft] += Number(r.consumption || 0)
          nozzlePourBack[ft] += Number(r.pour_back || 0)
        }

        dayDispensed[ft] += ftDispensed
        totalSales += ftDispensed * price
      }
    }

    // Step 2: Deduct consumption/pour-back. Use db.consumption when available,
    // fall back to nozzle-level consumption for fuel types without form entries.
    for (const ft of fuelTypes) {
      const price = Number(lastPrices[ft] || 0)
      const dbConsumed = dayConsumption.byFuelType?.[ft]?.consumed || 0
      const dbPourBack = dayConsumption.byFuelType?.[ft]?.pourBack || 0
      const hasDbEntries = dbConsumed > 0 || dbPourBack > 0
      const ftConsumed = hasDbEntries ? dbConsumed : nozzleConsumed[ft]
      const ftPourBack = hasDbEntries ? dbPourBack : nozzlePourBack[ft]
      totalSales -= (ftConsumed + ftPourBack) * price
    }
