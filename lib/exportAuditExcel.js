...
function writeSalesCash(wb, report, stationName, startDate, endDate, refs) {
  ...
  for (let ftIdx = 0; ftIdx < fuelTypes.length; ftIdx++) {
    ...
    const ROWS_PER_PERIOD = 10 // 8 pumps + 2 blank rows for future entries
    ...
    for (let ri = 0; ri < summary.rows.length; ri++) {
      ...
      // Fill remaining allocated rows with formulas (for future meter readings)
      const rowsUsed = row - periodStartRow
      for (let i = rowsUsed; i < ROWS_PER_PERIOD; i++) {
