...
// ── Styles ───────────────────────────────────────────────────────────────────
const ACCENT_FILL = { type: 'pattern', pattern: 'solid', fgColor: { theme: 5, tint: 0.4 }, bgColor: { indexed: 64 } }
...
const BORDER = { top: THIN('FF000000'), left: THIN('FF000000'), bottom: THIN('FF000000'), right: THIN('FF000000') }
 ...
 * @param {number} delivery.chartUllage
 * @param {number} delivery.chartLiquidHeight
 * @param {number} delivery.stationUllage
 * @param {number} delivery.stationLiquidHeight
 * @param {number} delivery.depotUllage - maps to "Waybill"
 * @param {number} delivery.depotLiquidHeight - maps to "Waybill"
 * @param {number} delivery.firstCompartment
 * @param {number} delivery.secondCompartment
 * @param {number} delivery.thirdCompartment
...
export async function exportProductReceiptExcel(delivery) {
  ...
  const d = delivery
  const qtyLoaded = (Number(d.firstCompartment) || 0) + (Number(d.secondCompartment) || 0) + (Number(d.thirdCompartment) || 0)
  ...
  const ullageChart = Number(d.chartUllage) || 0
  const ullageStation = Number(d.stationUllage) || 0
  const ullageWaybill = Number(d.depotUllage) || 0
  ...
  for (let i = 0; i < 3; i++) {
    ...
    sc(ws, r, 3, ullageChart, FONT_MAIN_BLACK, NO_FILL, ALIGN_CENTER)
    sc(ws, r, 4, ullageStation, FONT_MAIN, NO_FILL, ALIGN_CM)
    sc(ws, r, 5, ullageWaybill, FONT_MAIN, NO_FILL, ALIGN_CM)
  }
  ...
  const lhChart = Number(d.chartLiquidHeight) || 0
  const lhStation = Number(d.stationLiquidHeight) || 0
  const lhWaybill = Number(d.depotLiquidHeight) || 0
  ...
  for (let i = 0; i < 3; i++) {
    ...
    sc(ws, r, 3, lhChart, FONT_MAIN_BLACK, NO_FILL, ALIGN_CENTER)
    sc(ws, r, 4, lhStation, FONT_MAIN, NO_FILL, ALIGN_CM)
    sc(ws, r, 5, lhWaybill, FONT_MAIN, NO_FILL, ALIGN_CM)
  }
  ...
  const buildComparisonTable = (startRow, sn, title, col1Label, col2Label) => {
    ...
    sc(ws, hr, 5, 'Diff', FONT_SUMMARY, sn === 3 ? NO_FILL : YELLOW_FILL, ALIGN_LM)
    sc(ws, hr, 6, sn === 3 ? null : 'MF', FONT_SUMMARY, NO_FILL, ALIGN_LM)
    sc(ws, hr, 7, 'Diff', FONT_SUMMARY, sn === 3 ? NO_FILL : YELLOW_FILL, ALIGN_LM)
    ...
    for (let i = 0; i < 3; i++) {
      ...
      sc(ws, dr, 3, 0, FONT_MAIN_BLACK, NO_FILL, ALIGN_CENTER)
      sc(ws, dr, 4, 0, FONT_MAIN, NO_FILL, ALIGN_CM)
      const diffCell = sc(ws, dr, 5, { formula: `C${dr}-D${dr}` }, FONT_SUMMARY, YELLOW_FILL, ALIGN_RM)
      sc(ws, dr, 6, 0, FONT_SUMMARY, NO_FILL, ALIGN_RM)
      sc(ws, dr, 7, { formula: `E${dr}*F${dr}` }, FONT_SUMMARY, YELLOW_FILL, ALIGN_RM)
    }
  ...
  buildComparisonTable(32, 1, 'Chart vs Loading', 'Chart ', 'Loading')
  // 2) Chart vs Station — rows 40-47
  buildComparisonTable(40, 2, 'Chart vs Station', 'Chart', 'Station')
  // 3) Loading vs Station — rows 48-53 (no yellow on Diff header, no MF label)
  buildComparisonTable(48, 3, 'Loading vs Station', 'Loading', 'Station')
  ...
  for (let i = 0; i < 3; i++) {
    ...
    sc(ws, r, 5, 0, FONT_MAIN_BLACK, NO_FILL, ALIGN_CENTER)
    sc(ws, r, 6, 0, FONT_MAIN, NO_FILL, ALIGN_CM)
    sc(ws, r, 7, { formula: `E${r}-F${r}` }, FONT_SUMMARY, YELLOW_FILL, ALIGN_CM)
    sc(ws, r, 8, 0, FONT_SUMMARY, NO_FILL, ALIGN_CM)
