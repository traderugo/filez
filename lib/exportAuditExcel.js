 ...
 */
// Section config for the template (all 0-indexed rows)
const SECTIONS = {
...
export async function exportAuditExcel({
  ...
  // Fetch template — prefer admin-uploaded URL, fall back to public/templates
  let buf
  if (templateUrl) {
    try {
  ...
  const warnings = []
  fillSalesCash(wb, report, nozzles, stationName, startDate, endDate, fmtDate, warnings)
...
function fillSalesCash(wb, report, nozzles, stationName, startDate, endDate, fmtDate, warnings) {
