...
export async function GET(request) {
  try {
    ...
    if (id) {
      const { data } = await supabase.from(TABLE).select('*, users:created_by(name), product:product_id(id, product_name)').eq('id', id).eq('org_id', user.org_id).is('deleted_at', null).single()
...
export async function POST(request) {
  try {
    ...
    await logActivity(supabase, { orgId: user.org_id, userId: user.id, userName: user.name || user.email, content: 'added a lube sales entry', actionType: 'created_entry' })
    return NextResponse.json({ entry: data })
  } catch {
    return NextResponse.json({ error: 'Server error' }, { status: 500 })
  }
...
export async function PATCH(request) {
  try {
    ...
    await logActivity(supabase, { orgId: user.org_id, userId: user.id, userName: user.name || user.email, content: 'updated a lube sales entry', actionType: 'updated_entry' })
    return NextResponse.json({ entry: data })
  } catch {
    return NextResponse.json({ error: 'Server error' }, { status: 500 })
  }
