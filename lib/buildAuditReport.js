...
function buildSalesCashPosition({ sales, lodgements, consumption, nozzles, banks, fuelTypes, customerMap, startDate, endDate }) {
  ...
  // Distribute consumption into meter row amounts so that row amounts
  // reflect actual sales (dispensed - consumption) × price, not dispensed × price.
  // Consumption is allocated proportionally by dispensed volume across rows.
  for (const ft of fuelTypes) {
    const rows = fuelData[ft].rows
    const totalDisp = rows.reduce((s, r) => s + r.dispensed, 0)
    const totalConsumption = consumptionByFuel[ft].totalConsumedQty
    if (!totalDisp || !totalConsumption) continue

    for (const row of rows) {
      const ratio = row.dispensed / totalDisp
      const consumptionShare = totalConsumption * ratio
      // Update per-pump actual
      for (const pump of row.pumps) {
        const pumpRatio = pump.dispensed / totalDisp
        pump.actual = pump.dispensed - (totalConsumption * pumpRatio)
      }
      row.actual = row.dispensed - consumptionShare
      row.amount = row.actual * row.price
    }
  }
  ...
  for (const ft of fuelTypes) {
    const rows = fuelData[ft].rows
    // Use actual qty (dispensed - consumption) when available, raw dispensed otherwise
    const totalDispensed = rows.reduce((s, r) => s + (r.actual ?? r.dispensed), 0)
    // totalAmount now reflects actual sales (consumption already deducted in row amounts)
    ...
    // Consumption is already factored into row amounts, so zero here to avoid double-counting
    const totalConsumedQty = 0
    const totalConsumedAmt = 0
...
function buildLodgementSheet({ sales, lodgements, consumption, nozzles, banks, fuelTypes, startDate, endDate }) {
  ...
  const rows = dateStrings.map((date, idx) => {
    ...
    // Step 1: Calculate dispensed amounts from all entries
    let totalSales = 0
    const dayDispensed = {}
    for (const ft of fuelTypes) dayDispensed[ft] = 0
    ...
    // Step 2: Deduct consumption/pour-back once per day (from db.consumption only)
    for (const ft of fuelTypes) {
      const price = Number(lastPrices[ft] || 0)
      const ftConsumed = dayConsumption.byFuelType?.[ft]?.consumed || 0
      const ftPourBack = dayConsumption.byFuelType?.[ft]?.pourBack || 0
