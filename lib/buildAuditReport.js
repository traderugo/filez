...
function buildSalesCashPosition({ sales, lodgements, consumption, nozzles, banks, fuelTypes, customerMap, startDate, endDate }) {
  ...
  for (const ft of fuelTypes) {
    ...
    // Build a price-per-date map so entries without prices on a given day
    // still get the correct price from another entry on the same day.
    const priceDateMap = {}
    for (const e of rangeEntries) {
      const p = Number(e.prices?.[ft] || 0)
      if (p) priceDateMap[e.entryDate] = p
    }
    ...
    const processed = rangeEntries.map(e => {
      ...
      const price = Number(e.prices?.[ft] || 0) || priceDateMap[e.entryDate] || 0
...
function buildFuelMeterRows(fuelType, ftNozzles, pumpIds, entries, openings, initPrice) {
  ...
  // One row per entry that has readings. Each entry already represents a price period
  // (the user creates a separate entry each time the price changes), so amount = dispensed × that entry's price.
  // Former closing naturally becomes next opening because we track currentOpening across entries.
  const currentOpening = { ...openings }

  for (const entry of entries) {
    const hasClosings = pumpIds.some(pid => entry.closings[pid] != null)
    if (!hasClosings) continue

    let totalDispensed = 0
    const pumpDetails = ftNozzles.map(n => {
      const opening = currentOpening[n.id]
      const closing = entry.closings[n.id] != null ? entry.closings[n.id] : opening
    ...
    rows.push({ pumps: pumpDetails, dispensed: totalDispensed, price: entry.price, amount: totalDispensed * entry.price })

    for (const pid of pumpIds) {
      if (entry.closings[pid] != null) currentOpening[pid] = entry.closings[pid]
    }
  }
