...
function writeSalesCash(wb, report, stationName, startDate, endDate, refs) {
  ...
  // Helper: write a reconciliation label row (merged B-F) + yellow value in G
  // Spacer rows get only medium vertical edges, no horizontal borders
  const reconLabelBorder = {
    top: { style: 'thin' }, bottom: { style: 'thin' },
    left: { style: 'medium' }, right: { style: 'thin' },
  }
  const reconValueBorder = {
    top: { style: 'thin' }, bottom: { style: 'thin' },
    left: { style: 'thin' }, right: { style: 'medium' },
  }
  const reconLabel = (r, text) => {
    textCell(ws, r, 2, text, { font: FONT_BOLD, border: reconLabelBorder, alignment: { horizontal: 'left' } })
    // Merge B-F so label text spans across
    ws.mergeCells(r, 2, r, 6)
  }
  const reconValue = (r, formula) => {
    formulaYellow(ws, r, 7, formula, { font: FONT_BOLD, fill: YELLOW_FILL, border: reconValueBorder, alignment: { horizontal: 'center' } })
  }
  const reconSpacer = (r) => {
  ...
  // Date value in col F
  textCell(ws, row, 6, fmtDateDisplay(startDate), { font: FONT_BOLD, border: { bottom: { style: 'medium' } }, alignment: { horizontal: 'left' } })
  ...
  reconLabel(row, 'Expected Sales from Products for the Period ')
  reconValue(row, expectedFormula)
  row++
  ...
  reconLabel(row, 'Less expenses for the Period(Approved)')
  reconValue(row, "'9.Expenses for the Month'!E38")
  row++
  ...
  reconLabel(row, 'Net Expected Sales for the Period')
  reconValue(row, `G${refs.expectedSalesRow}-G${refs.expensesRow}`)
  row++
  ...
  reconLabel(row, 'Total lodgement for the Period')
  reconValue(row, `'4.Lodgement Sheet'!T${refs.lodgementTotalRow || 172}`)
  row++
  ...
  // Total POS
  reconLabel(row, 'Total POS for the Period')
  reconValue(row, `'4.Lodgement Sheet'!Q${refs.lodgementTotalRow || 172}`)
  ...
  reconLabel(row, 'Opening Balance of Debit/Credit customers')
  reconValue(row, "'Customers'' Ledger'!C43")
  ...
  reconLabel(row, 'Closing Balance of Debit/Credit customers')
  reconValue(row, "'Customers'' Ledger'!J43")
  ...
  // Overage/Shortage
  reconLabel(row, 'Overage/Shortage')
  reconValue(row, `(G${refs.totalLodgementRow}+G${refs.totalPOSRow}+G${refs.openingBalRow}-G${refs.closingBalRow})-G${refs.netExpectedRow}`)
}
