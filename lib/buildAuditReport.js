...
function buildSalesCashPosition({ sales, lodgements, nozzles, banks, fuelTypes, customerMap, startDate, endDate }) {
  ...
  // DEBUG: log consumption totals per fuel type
  if (typeof window !== 'undefined') {
    for (const ft of fuelTypes) {
      const c = consumptionByFuel[ft]
      console.log(`[SalesCash] ${ft}: consumedQty=${c.totalConsumedQty}, consumedAmt=${c.totalConsumedAmt}, pourBackQty=${c.totalPourBackQty}, consumedEntries=${c.consumed.length}`)
    }
    // Also log what consumptionResult returned per date
    let inRangeCons = 0
    for (const dayData of consumptionResult.dates) {
      for (const ft of fuelTypes) {
        const ftEntries = dayData.byFuelType[ft]?.entries || []
        for (const e of ftEntries) inRangeCons += Number(e.quantity || 0)
      }
    }
    console.log(`[SalesCash] Total consumption in date range (${startDate} to ${endDate}): ${inRangeCons}`)
  }
