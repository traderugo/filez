...
function calculateDateRangeConsumption(allSales, startDate, endDate) {
  ...
  // DEBUG: log consumption data found in nozzle readings
  if (typeof window !== 'undefined') {
    let totalCons = 0, totalPb = 0
    for (const sale of allSales) {
      for (const r of (sale.nozzleReadings || [])) {
        totalCons += Number(r.consumption || 0)
        totalPb += Number(r.pour_back || 0)
      }
    }
    if (totalCons > 0 || totalPb > 0) {
      console.log('[ConsumptionCalc] Found consumption:', totalCons, 'pourBack:', totalPb, 'across', allSales.length, 'entries')
    } else {
      console.warn('[ConsumptionCalc] NO consumption/pourBack found in', allSales.length, 'entries. Sample nozzle reading:', allSales[0]?.nozzleReadings?.[0])
    }
  }
