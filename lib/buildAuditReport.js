...
function buildFuelMeterRows(fuelType, nozzles, allSorted, prevEntry, dateStrings, entriesByDate) {
  ...
  for (const date of dateStrings) {
    ...
    if (dayEntries.length === 1) {
      ...
      // Entry 0: merge into accumulated row (or emit standalone if no open row)
      //          — this is the proven 2-entry logic.
      // Entry 1+: treat like individual single-entry days
      //          — accumulate at same price, emit on price change.
      for (let i = 0; i < dayEntries.length; i++) {
        const entry = dayEntries[i]
        const entryPrice = Number(entry.prices?.[fuelType] || 0) || currentPrice

        if (i === 0 && hasOpenRow) {
          // Merge entry 0's closing into accumulated row, emit at old price
          for (const n of ftNozzles) {
            const r = (entry.nozzleReadings || []).find(r => r.pump_id === n.id)
            if (r) lastClosing[n.id] = Number(r.closing_meter || 0)
          }
          ...
          // No open row — emit entry 0 as its own row
          for (const n of ftNozzles) {
            const r = (entry.nozzleReadings || []).find(r => r.pump_id === n.id)
            if (r) lastClosing[n.id] = Number(r.closing_meter || 0)
          }
          ...
          // Entry 1+: same logic as single-entry days
          if (hasOpenRow && entryPrice !== currentPrice) {
            emitRow(currentPrice)
          }
          for (const n of ftNozzles) {
