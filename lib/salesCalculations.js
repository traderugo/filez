 ...
 *   - consumption override from db.consumption entries
 *
 ...
 * @param {Object} [params.dayConsumption] - From consumptionCalculations: { byFuelType: { [ft]: { consumed, pourBack } } }
 * @returns {{ dayFuelTotals: Object, entryGroups: Array }}
 */
function computeDayFuelTotals({ helperEntries, dateEntries, fuelTypes, allSorted, date, dayConsumption }) {
  const dayFuelTotals = {}
  for (const ft of fuelTypes) {
  ...
  // Apply consumption override from db.consumption entries
  ...
  if (dayConsumption) {
    ...
    // Propagate override to per-entry subtotals
