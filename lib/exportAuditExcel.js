...
function writeStockPosition(wb, report, stationName, startDate, endDate, refs) {
  ...
  for (const ft of fuelTypes) {
    ...
    // Border styles for stock position: medium left on C, medium right on D
    const stockBorderCD = {
      top: { style: 'thin' }, bottom: { style: 'thin' },
      left: { style: 'medium' }, right: { style: 'medium' },
    }
    const stockLabelStyle = { font: FONT_BOLD, border: stockBorderCD, alignment: { horizontal: 'left' } }
    const stockValueStyle = { font: FONT_BOLD, border: stockBorderCD, alignment: { horizontal: 'center' } }
    const stockDataStyle = { font: FONT, border: stockBorderCD, alignment: { horizontal: 'center' } }
    ...
    // Section header: yellow fill, medium box border
    textCell(ws, row, 3, `${ft} STOCK POSITION`, { font: FONT_12_BOLD, fill: YELLOW_FILL, border: { top: { style: 'medium' }, bottom: { style: 'medium' }, left: { style: 'medium' }, right: { style: 'thin' } }, alignment: { horizontal: 'left' } })
    textCell(ws, row, 4, "Per Manager's Computation", { font: FONT_12_BOLD, fill: YELLOW_FILL, border: { top: { style: 'medium' }, bottom: { style: 'medium' }, left: { style: 'thin' }, right: { style: 'medium' } }, alignment: { horizontal: 'left' } })
    ...
    // Blank spacer before OVERAGE — medium bottom border (separator line)
    ws.getRow(row).getCell(3).border = { left: { style: 'medium' }, bottom: { style: 'medium' } }
    ws.getRow(row).getCell(4).border = { right: { style: 'medium' }, bottom: { style: 'medium' } }
    ws.getRow(row).getCell(3).font = FONT
    ws.getRow(row).getCell(4).font = FONT
    row++

    // OVERAGE/(SHORTAGE) — medium box border (ACCT_PAREN per reference)
    textCell(ws, row, 3, 'OVERAGE/(SHORTAGE)', { font: FONT_BOLD, border: { top: { style: 'medium' }, bottom: { style: 'medium' }, left: { style: 'medium' }, right: { style: 'thin' } }, alignment: { horizontal: 'left' } })
    formulaCell(ws, row, 4, `D${dispRow}-D${qtySoldRow}`, { font: FONT_BOLD, border: { top: { style: 'medium' }, bottom: { style: 'medium' }, left: { style: 'thin' }, right: { style: 'medium' } }, alignment: { horizontal: 'center' } }, ACCT_PAREN)
    row++

    // 3 spacer rows with just vertical edges
    for (let s = 0; s < 3; s++) {
      spacerEdges(row)
      row++
    }

    // EXPECTED LITRES (NUM_FMT per reference)
    textCell(ws, row, 3, 'EXPECTED LITRES', stockLabelStyle)
    if (fMap.prodExpCol) {
      formulaCell(ws, row, 4, `'8.Product Received'!${fMap.prodExpCol}52`, stockDataStyle, NUM_FMT)
    } else {
      numCell(ws, row, 4, t.expectedLitres, stockDataStyle, NUM_FMT)
    }
    ...
    textCell(ws, row, 3, 'ACTUAL LITRES RECEIVED', stockLabelStyle)
    if (fMap.prodActCol) {
      formulaCell(ws, row, 4, `'8.Product Received'!${fMap.prodActCol}52`, stockDataStyle)
    } else {
      numCell(ws, row, 4, t.actualLitresReceived, stockDataStyle)
    }
    ...
    textCell(ws, row, 3, 'OVERAGE/(SHORTAGE) - TRUCK DRIVER', stockLabelStyle)
    formulaCell(ws, row, 4, `D${actLitresRow}-D${expLitresRow}`, stockValueStyle)
    row++
