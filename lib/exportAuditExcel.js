  ...
  for (let ftIdx = 0; ftIdx < fuelTypes.length; ftIdx++) {
    ...
    for (let ri = 0; ri < summary.rows.length; ri++) {
      ...
      for (const pump of period.pumps) {
        ...
        // E = D - C (dispensed) — yellow formula, ACCT_FMT_PLAIN per reference
        const eRef = `D${row}-C${row}`
        formulaYellow(ws, row, 5, eRef, dataStyleYellow(), ACCT_FMT_PLAIN)

        numCell(ws, row, 6, period.price, dataStyle(), 'General')

        // G = E * F (amount) — yellow formula, ACCT_FMT_PLAIN per reference
        const gRef = `E${row}*F${row}`
        formulaYellow(ws, row, 7, gRef, dataStyleYellow(), ACCT_FMT_PLAIN)
        row++
      }
    }
...
function writeStockPosition(wb, report, stationName, startDate, endDate, refs) {
  ...
  for (const ft of fuelTypes) {
    ...
    // SUPPLIES DURING THE PERIOD — formula referencing Product Received (NUM_FMT per reference)
    textCell(ws, row, 3, 'SUPPLIES DURING THE PERIOD ', { font: FONT_BOLD, border: BORDER_THIN, alignment: { horizontal: 'left' } })
    if (fMap.prodActCol) {
      formulaCell(ws, row, 4, `'8.Product Received'!${fMap.prodActCol}52`, dataStyle(), NUM_FMT)
    } else {
      numCell(ws, row, 4, t.supply, dataStyle(), NUM_FMT)
    }
    row++
    ...
    formulaCell(ws, row, 4, `D${stockAvailRow}-D${closingRow}`, { font: FONT_BOLD, border: BORDER_THIN, alignment: { horizontal: 'center' } }, NUM_FMT)
    ...
    // OVERAGE/(SHORTAGE) = DISPENSED - SOLD (ACCT_PAREN per reference)
    textCell(ws, row, 3, 'OVERAGE/(SHORTAGE)', { font: FONT_BOLD, border: BORDER_THIN, alignment: { horizontal: 'left' } })
    formulaCell(ws, row, 4, `D${dispRow}-D${qtySoldRow}`, { font: FONT_BOLD, border: BORDER_THIN, alignment: { horizontal: 'center' } }, ACCT_PAREN)
    row++

    // Spacer rows
    row += 3

    // EXPECTED LITRES — formula referencing Product Received (NUM_FMT per reference)
    textCell(ws, row, 3, 'EXPECTED LITRES', { font: FONT_BOLD, border: BORDER_THIN, alignment: { horizontal: 'left' } })
    if (fMap.prodExpCol) {
      formulaCell(ws, row, 4, `'8.Product Received'!${fMap.prodExpCol}52`, dataStyle(), NUM_FMT)
    } else {
      numCell(ws, row, 4, t.expectedLitres, dataStyle(), NUM_FMT)
    }
