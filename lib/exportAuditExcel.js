...
function writeSalesCash(wb, report, stationName, startDate, endDate, refs, logoId) {
  ...
  const titleMonth = startDate ? new Date(startDate + 'T00:00:00').toLocaleDateString('en-US', { month: 'long', year: 'numeric' }).toUpperCase() : ''
  const titleText = `CASH/SALES RECONCILIATION AS  AT ${titleMonth}`
  ...
  for (let ftIdx = 0; ftIdx < fuelTypes.length; ftIdx++) {
    ...
    // Empty closing row (T:thin, B:medium) then 2 spacer rows with outer edges only
    if (ftIdx < fuelTypes.length - 1) {
      // Closing border row — bottom medium to close the section
      closingRows.push(row)
      row++
      // 2 spacer rows — track for post-fillGrid cleanup
