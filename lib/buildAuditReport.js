...
function buildSalesCashPosition({ sales, lodgements, consumption, nozzles, banks, fuelTypes, customerMap, startDate, endDate }) {
  ...
  for (const ft of fuelTypes) {
    ...
    const processed = rangeEntries.map(e => {
      ...
      return { price: Number(e.prices?.[ft] || 0), closings, date: e.entryDate, createdAt: e.createdAt }
    })
    ...
    if (ft === 'PMS') {
      ...
      processed.forEach((p, i) => console.log(`  [${i}] date=${p.date}, price=${p.price}, closings=`, p.closings))
