...
export async function exportProductReceiptExcel(delivery) {
  ...
  const summaryBlankRow = (r) => {
    ...
    for (let c = 2; c <= 7; c++) {
      const cell = ws.getCell(r, c)
      cell.font = FONT_SEG; cell.fill = NO_FILL; cell.alignment = { horizontal: 'left' }; cell.border = BORDER_THIN
    }
  }
  ...
  const buildComparisonTable = (startRow, sn, title, col1Label, col2Label, col1Vals, col2Vals, mfVals) => {
    ...
    sc(ws, startRow, 2, sn, FONT_SUMMARY_BOLD, NO_FILL, ALIGN_RM, B)
    for (let c = 3; c <= 7; c++) sc(ws, startRow, c, title, FONT_SUMMARY_BOLD, NO_FILL, ALIGN_CM, B)
    ws.mergeCells(`C${startRow}:G${startRow}`)

    // Column headers
    const hr = startRow + 1
    ws.getRow(hr).height = 15.75
    sc(ws, hr, 2, null, FONT_SUMMARY, NO_FILL, ALIGN_LM, B)
    sc(ws, hr, 3, col1Label, FONT_SUMMARY, NO_FILL, ALIGN_LM, B)
    sc(ws, hr, 4, col2Label, FONT_SUMMARY, NO_FILL, ALIGN_LM, B)
    sc(ws, hr, 5, 'Diff', FONT_SUMMARY, mfVals ? YELLOW_FILL : NO_FILL, ALIGN_LM, B)
    sc(ws, hr, 6, mfVals ? 'MF' : null, FONT_SUMMARY, NO_FILL, ALIGN_LM, B)
    sc(ws, hr, 7, 'Diff', FONT_SUMMARY, mfVals ? YELLOW_FILL : NO_FILL, ALIGN_LM, B)

    // Data rows (3 compartments)
    for (let i = 0; i < 3; i++) {
      const dr = startRow + 2 + i
      ws.getRow(dr).height = 16.5
      sc(ws, dr, 2, null, FONT_SUMMARY, NO_FILL, ALIGN_LM, B)
      sc(ws, dr, 3, Number(col1Vals[i]) || 0, FONT_MAIN_BLACK, NO_FILL, ALIGN_CENTER, B)
      sc(ws, dr, 4, Number(col2Vals[i]) || 0, FONT_MAIN, NO_FILL, ALIGN_CM, B)
      sc(ws, dr, 5, { formula: `C${dr}-D${dr}` }, FONT_SUMMARY, mfVals ? YELLOW_FILL : NO_FILL, ALIGN_RM, B)
      const mfCell = sc(ws, dr, 6, mfVals ? (Number(mfVals[i]) || 0) : 0, FONT_SUMMARY, NO_FILL, ALIGN_RM, B)
      if (mfVals) mfCell.numFmt = '0.00'
      sc(ws, dr, 7, { formula: `E${dr}*F${dr}` }, FONT_SUMMARY, mfVals ? YELLOW_FILL : NO_FILL, ALIGN_RM, B)
    }

    // Totals row
    const tr = startRow + 5
    ws.getRow(tr).height = 15.75
    sc(ws, tr, 2, null, FONT_SUMMARY, NO_FILL, ALIGN_LM, B)
    sc(ws, tr, 3, null, FONT_SUMMARY, NO_FILL, ALIGN_LM, B)
    sc(ws, tr, 4, null, FONT_SUMMARY, NO_FILL, ALIGN_LM, B)
    const d1 = startRow + 2, d3 = startRow + 4
    sc(ws, tr, 5, { formula: `SUM(E${d1},E${d1 + 1},E${d3})` }, FONT_SUMMARY, NO_FILL, ALIGN_RM, B)
    sc(ws, tr, 6, null, FONT_SUMMARY, NO_FILL, ALIGN_LM, B)
    sc(ws, tr, 7, { formula: `SUM(G${d1},G${d1 + 1},G${d3})` }, FONT_SUMMARY_BOLD, NO_FILL, ALIGN_RM, B)
  ...
  ws.getCell(55, 2).border = BORDER_THIN
  ...
  for (let i = 0; i < lhHeaders.length; i++) {
    sc(ws, 59, i + 2, lhHeaders[i], FONT_SUMMARY_BOLD, NO_FILL, ALIGN_CM, B)
  }
  sc(ws, 59, 10, null, FONT_SUMMARY, NO_FILL, ALIGN_LM, B)

  // Rows 60-62: Data (3 compartments, B60:B62 and C60:C62 merged)
  for (let i = 0; i < 3; i++) {
    const r = 60 + i
    ws.getRow(r).height = 16.5
    sc(ws, r, 2, 1, FONT_SUMMARY_BOLD, NO_FILL, ALIGN_CM, B)
    sc(ws, r, 3, d.truckNumber || '', FONT_SUMMARY_BOLD, NO_FILL, ALIGN_CM, B)
    sc(ws, r, 4, null, FONT_MAIN, NO_FILL, ALIGN_CM, B)
    sc(ws, r, 5, Number(cLH[i]) || 0, FONT_MAIN_BLACK, NO_FILL, ALIGN_CENTER, B)
    sc(ws, r, 6, Number(sLH[i]) || 0, FONT_MAIN, NO_FILL, ALIGN_CM, B)
    sc(ws, r, 7, { formula: `E${r}-F${r}` }, FONT_SUMMARY, YELLOW_FILL, ALIGN_CM, B)
    const mfLhCell = sc(ws, r, 8, Number(lhMF[i]) || 0, FONT_SUMMARY, NO_FILL, ALIGN_CM, B)
    mfLhCell.numFmt = '0.00'
    sc(ws, r, 9, { formula: `H${r}*G${r}` }, FONT_SUMMARY, YELLOW_FILL, ALIGN_CM, B)
    sc(ws, r, 10, null, FONT_SUMMARY, NO_FILL, ALIGN_LM, B)
  }
  ...
  ws.getCell(63, 6).border = BORDER_THIN
