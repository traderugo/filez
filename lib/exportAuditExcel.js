...
function writeSalesCash(wb, report, stationName, startDate, endDate, refs) {
  ...
  for (let ftIdx = 0; ftIdx < fuelTypes.length; ftIdx++) {
    ...
    // Write pump data into fixed allocation
    const pumpAlloc = Math.max(alloc.pumpRows, summary.rows.reduce((s, p) => s + p.pumps.length, 0))
    let pumpsWritten = 0
    for (const period of summary.rows) {
    ...
    for (let i = pumpsWritten; i < pumpAlloc; i++) {
      formulaYellow(ws, row, 5, `D${row}-C${row}`, dataStyleYellow(), ACCT_FMT_PLAIN)
      formulaYellow(ws, row, 7, `E${row}*F${row}`, dataStyleYellow(), ACCT_FMT_PLAIN)
      row++
    }
    ...
    const pourBackAlloc = Math.max(alloc.pourBackRows, (summary.consumption?.pourBackByPrice || []).length || 1)
    ...
    const consumeAlloc = Math.max(alloc.consumeRows, (summary.consumption?.consumedByPrice || []).length)
    ...
    // Fill remaining consumption rows
