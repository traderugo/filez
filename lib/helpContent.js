...
export const HELP_SECTIONS = [
    ...
    blocks: [
      ...
      { p: 'When you save an entry it is written to this device and added to a queue. The entry is real and reports include it immediately. It is not yet on the server, and it is not yet on any other device.' },
      {
        defs: [
          ...
          ['Clear queue', 'Throws the queue away. Entries that were never pushed are deleted with it, because the queue was the only record that they existed.'],
        ],
      },
      { note: 'If entries seem to have vanished, check the pending count before anything else. Data entered on a phone that was never pushed will not appear on a laptop, and clearing the queue on that phone destroys it for good. Push before switching devices, and pull after.' },
  ...
  },
  {
    ...
    blocks: [
      {
        defs: [
          ['Entries are missing on another device', 'They were never pushed. Go back to the device they were entered on and push, then pull here. Do not clear the queue on that device first, as that deletes them.'],
          ...
          ['A day reopened with entries in a strange order', 'Fixed. Entries saved together now keep their order when the day is reopened. Days saved before the fix may still show the old order until re-saved.'],
        ],
      },
