...
export function buildAnalyticsReport({ sales, receipts, lodgements, nozzles, tanks, banks, startDate, endDate }) {
  ...
  const series = range.dates.map(({ date, entries: helperEntries, hasEntry }) => {
    ...
    // Variance per fuel as % of dispensed — standard convention:
    //   positive = overage, negative = shortage. Tolerance is ±1.25%.
    ...
    for (const ft of fuelTypes) {
      ...
      varianceByFuel[ft] = dispensed > 0 ? (actualOvsh / dispensed) * 100 : 0
    }
  ...
  // Per-fuel period totals
  const totalSalesByFuel = {}
  const totalVolumeByFuel = {}
  const avgDailyRevenueByFuel = {}
  const avgDailyVolumeByFuel = {}
  for (const ft of fuelTypes) {
    const s = series.reduce((acc, d) => acc + (d.revenueByFuel[ft] || 0), 0)
    const v = series.reduce((acc, d) => acc + (d.volumeByFuel[ft] || 0), 0)
    totalSalesByFuel[ft] = s
    totalVolumeByFuel[ft] = v
    avgDailyRevenueByFuel[ft] = s / dayCount
    avgDailyVolumeByFuel[ft] = v / dayCount
  }
  ...
  return {
    ...
    summary: {
      ...
      totalSalesByFuel,
      totalVolumeByFuel,
      avgDailyRevenueByFuel,
      avgDailyVolumeByFuel,
    },
    fuelMix,
    series,
    stockSeries,
