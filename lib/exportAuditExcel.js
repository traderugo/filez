...
function writeGuideline(wb) {
  ...
  ws.getRow(8).getCell(3).value = {
    richText: [
      { font: FONT, text: '1. This report should be sent to the Internal Audit department not later than 4 working days after the end of the reporting month. For example, the report for September 2018 should be sent latest by 4th of October 2018.\n2. This template is a replica of the Station record (The daily Sales, Expenses, Stock, Lodgment, Product received and Consumption book). Thus, at every point, the data captured herein must agree with what is captured in the Station record.\n3.Sheet 2 (Sales/Cash Reconciliaition) contains the Sales/Cash reconciliation and must be balanced before sending it for review to the Internal Audit department.\n4.At no point should there be any difference between the data captured in this template and what is captured in the Station book. This will be reviewed during audit visit for any discrepancy/inconsistency.\n5.Sheet 3 (Stock Position) has figures that are linked and protected and as such no data should be keyed-in apart from the respective month and period. The sheet 3 is the result of the stock position of the Station at the end of the month.\n6.Some of the rows are protected expecially the ones highlighted in ' },
      { font: FONT_BOLD, text: '"Yellow"' },
  ...
  ws.getRow(10).getCell(3).value = {
    richText: [
      { font: FONT, text: 'All the sheets must be populated with accurate figure in order to determine the Actual Cash and Stock Position of the Station on a monthly basis. This template will be used by the Internal Auditor in carrying out a detailed audit of the Stations activities for the respective months. If you require any clarification on how to navigate the template, kindly reach out to any of the Internal Auditors.\n\nJohn Oruh---08056799340\n\nEdmond Anifowose--08035127657\n\nEvelyn Chukwulobe--08060109252\n\n' },
    ]
  }
...
function writeSalesCash(wb, report, stationName, startDate, endDate, refs) {
  ...
  textCell(ws, 3, 3, stationName || '', { font: FONT, fill: BLUE_FILL, alignment: { horizontal: 'left' } })
  ...
  for (let ftIdx = 0; ftIdx < fuelTypes.length; ftIdx++) {
    ...
    // 2 spacer rows before next fuel type — track for post-fillGrid cleanup
