...
const SECTION_BG = 'FFBDD6EE' // section title rows
const LABEL_BG   = 'FFB4C6E7' // "Station", "Date", "Amount (₦)", "Bank" label cells in col B
const HEADER_BG  = 'FFF4B083' // orange — column headers ("Price 1", "Opening stock", etc.)
const FUEL_BG    = 'FFC5E0B3' // green — fuel type labels in col A (PMS/AGO/DPK)
const YELLOW_BG  = 'FFFFFF00' // total/formula output cells (K column on fuel rows, Reason cell, etc.)
...
function buildSheet(ws, sheet, stationName) {
  ...
  // ─── R4: Fuel headers (orange) ──────────────────────────────────────
  setCell(ws, 4, 1, '', { fill: fill(HEADER_BG), border: headerBorder(true, false) })
  ...
  for (const ft of ['PMS', 'AGO', 'DPK']) {
    ...
    setFormula(ws, row, 10, `C${row}+G${row}`, f.totalActualLtrs || 0, { border: fuelBorder })
  ...
  // ─── R8: LUBES row (B8:J8 merged) ──────────────────────────────────
  setCell(ws, 8, 1, 'LUBES', {
    font: FONT, fill: fill(FUEL_BG),
    border: { left: thick(), top: thin(), bottom: thin(), right: thin() },
  })
  setCell(ws, 8, 2, '', { border: dataBorder(false, false) })
  for (let c = 3; c <= 10; c++) {
    setCell(ws, 8, c, '', { border: { top: thin(GRAY), bottom: thin(GRAY) } })
  }
  ...
  // ─── R10: stock headers (orange, only A-G; H10:K13 merged empty) ────
  setCell(ws, 10, 1, '', { fill: fill(HEADER_BG), border: headerBorder(true, false) })
  ...
  // H10:K13 — merged empty cell (reference uses cols A-G for stock)
  setCell(ws, 10, 8, '', { border: { top: med(), bottom: thin(), left: thin(), right: thick() } })
  for (let r = 10; r <= 13; r++) {
    for (let c = 9; c <= 11; c++) {
      setCell(ws, r, c, '', { border: c === 11 ? { right: thick() } : {} })
    }
  }
  ...
  for (const ft of ['PMS', 'AGO', 'DPK']) {
    ...
    setFormula(ws, row, 5, `D${row}-C${row}`, (s.actualReceived || 0) - (s.waybill || 0), { border: bdr(topB, botB) })
    setFormula(ws, row, 6, `C${fuelRow}+D${fuelRow}+E${fuelRow}+G${fuelRow}+H${fuelRow}+I${fuelRow}`, s.totalDispensed || 0, { border: bdr(topB, botB) })
  ...
  // ─── R15: recon headers ─────────────────────────────────────────────
  setCell(ws, 15, 1, '', { fill: fill(HEADER_BG), border: headerBorder(true, false) })
  ...
  for (const ft of ['PMS', 'AGO', 'DPK']) {
    ...
    cmtCell.value = { formula: `IF(D${row}>0, "Liquid height variance", "")`, result: msg }
    applyStyle(cmtCell, {
      font: FONT,
      alignment: { horizontal: 'left', vertical: 'middle' },
  ...
  // ─── R19: blank merged ──────────────────────────────────────────────
  for (let c = 1; c <= 11; c++) {
    setCell(ws, 19, c, '', { border: { left: c === 1 ? thick() : undefined, right: c === 11 ? thick() : undefined } })
  }
  ws.mergeCells(19, 1, 19, 11)

  // ─── R20: deposit + POS headers (orange) ────────────────────────────
  setCell(ws, 20, 1, '', { fill: fill(HEADER_BG), border: headerBorder(true, false) })
  ...
  setCell(ws, 21, 1, 'Amount (₦)', {
    font: FONT_BOLD, fill: fill(LABEL_BG),
    alignment: { horizontal: 'left', vertical: 'middle' },
  ...
  setCell(ws, 24, 1, '', { fill: fill(HEADER_BG), border: headerBorder(true, false) })
  ...
  setFormula(ws, 25, 2, `SUM(K5:K8)`, cash.totalExpectedSales, { border: { top: med(), bottom: med(), left: thin(), right: thin() } })
  setNum(ws, 25, 3, cash.prevDayCash, { border: { top: med(), bottom: med(), left: thin(), right: thin() } })
  setFormula(ws, 25, 4, `B21+C21+D21+E21`, cash.totalBankDeposit, { border: { top: med(), bottom: med(), left: thin(), right: thin() } })
  setFormula(ws, 25, 5, `SUM(F21:K21)`, cash.totalPOS, { border: { top: med(), bottom: med(), left: thin(), right: thin() } })
  setFormula(ws, 25, 6, `B25+C25-D25-E25`, cash.expectedCashAtHand, { border: { top: med(), bottom: med(), left: thin(), right: thin() } })
  setNum(ws, 25, 7, cash.actualCashAtHand, { border: { top: med(), bottom: med(), left: thin(), right: thin() } })
  setFormula(ws, 25, 8, `G25-F25`, cash.variance, { border: { top: med(), bottom: med(), left: thin(), right: thin() } })
  ...
  // ─── R26: blank merged ──────────────────────────────────────────────
  for (let c = 1; c <= 11; c++) {
    setCell(ws, 26, c, '', { border: { left: c === 1 ? thick() : undefined, right: c === 11 ? thick() : undefined } })
  }
  ws.mergeCells(26, 1, 26, 11)

  // ─── R27: budget headers ────────────────────────────────────────────
  setCell(ws, 27, 1, '', { fill: fill(HEADER_BG), border: headerBorder(true, false) })
  ...
  // ─── R32: blank merged ──────────────────────────────────────────────
  for (let c = 1; c <= 11; c++) {
    setCell(ws, 32, c, '', { border: { left: c === 1 ? thick() : undefined, right: c === 11 ? thick() : undefined } })
  }
  ws.mergeCells(32, 1, 32, 11)

  // ─── R33: competitor headers ────────────────────────────────────────
  setCell(ws, 33, 1, '', { fill: fill(HEADER_BG), border: headerBorder(true, false) })
  ...
  // ─── R38: lube headers ──────────────────────────────────────────────
  setCell(ws, 38, 1, '', { fill: fill(HEADER_BG), border: headerBorder(true, false) })
  ...
  if (lubeItems.length > 0) {
    ...
    setFormula(ws, totalRow, 5,  `SUM(E${firstLubeRow}:E${lastRow})`, sums.opening,  { border: totBdr })
    setFormula(ws, totalRow, 6,  `SUM(F${firstLubeRow}:F${lastRow})`, sums.supply,   { border: totBdr })
    setFormula(ws, totalRow, 7,  `SUM(G${firstLubeRow}:G${lastRow})`, sums.sales,    { border: totBdr })
    setFormula(ws, totalRow, 8,  `SUM(H${firstLubeRow}:H${lastRow})`, sums.closing,  { border: totBdr })
    setFormula(ws, totalRow, 9,  `SUM(I${firstLubeRow}:I${lastRow})`, sums.amount,   { border: totBdr })
    setFormula(ws, totalRow, 10, `SUM(J${firstLubeRow}:J${lastRow})`, sums.variance, { border: totBdr })
