...
function calculateDateRangeConsumption(allConsumption, startDate, endDate) {
  ...
  const dates = dateStrings.map(date => {
    // Deduplicate entries with same customerId + fuelType + quantity + isPourBack (from sync creating duplicates)
    const raw = entriesByDate[date] || []
    const seen = new Set()
    const dayEntries = raw.filter(e => {
      const key = `${e.customerId}_${e.fuelType}_${e.quantity}_${!!e.isPourBack}`
