...
export default function CustomerPaymentsListPage() {
  ...
  const hasConfig = useLiveQuery(
    () => ready && orgId ? db.customers.where('orgId').equals(orgId).count() : 0,
    [orgId, ready], 0
  )
  ...
  return (
    <div className="max-w-3xl px-4 sm:px-8 py-8">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-xl font-bold text-gray-900">Account Entries</h1>
              ...
              <div key={entry.id} className="py-3 flex items-center gap-3">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-500">
                    ...
                    {' · '}Paid: &#8358;{Number(entry.amountPaid).toLocaleString()} · Sales: &#8358;{Number(entry.salesAmount).toLocaleString()}
                  </p>
                </div>
                <Link href={`/dashboard/entries/customer-payments?${qs}&edit=${entry.id}`} className="flex items-center gap-1 text-xs font-medium text-blue-600 border border-blue-200 px-3 py-1.5 rounded hover:bg-blue-50">
