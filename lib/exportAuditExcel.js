...
function dateCell(ws, r, c, dateStr, style) {
  const cell = ws.getRow(r).getCell(c)
  if (dateStr) {
    cell.value = new Date(dateStr + 'T12:00:00')
  } else {
    cell.value = null
  }
...
export async function exportAuditExcel({
  ...
  // Pre-compute lodgement column positions so Sales/Cash Position can
  // reference them before the Lodgement Sheet is actually built.
  const lodgementBanks = report.lodgementSheet?.banks || []
  const displayBanks = lodgementBanks.filter(b => b.lodgement_type === 'pos' || b.lodgement_type === 'transfer')
  const bankStartCol = 9
  const bankCols = Math.max(displayBanks.length, 8)
  refs.lodgementQCol = colLetter(bankStartCol + bankCols)       // Total POS
  refs.lodgementTCol = colLetter(bankStartCol + bankCols + 3)   // Actual Lodgement
...
function writeSalesCash(wb, report, stationName, startDate, endDate, refs, logoId) {
  ...
  reconRow(row, 'Total lodgement for the Period', `'4.Lodgement Sheet'!${refs.lodgementTCol}${refs.lodgementTotalRow || 172}`)
  row++
  ...
  // Total POS
  reconRow(row, 'Total POS for the Period', `'4.Lodgement Sheet'!${refs.lodgementQCol}${refs.lodgementTotalRow || 172}`)
  ...
  // Formula: Net Expected - POS (includes transfers) - Lodgement + (Closing - Opening) credit balances
  textCell(ws, row, 2, 'Overage/Shortage', { font: FONT_BOLD, border: { left: { style: 'medium' } }, alignment: { horizontal: 'left' } })
  formulaYellow(ws, row, 7, `G${refs.netExpectedRow}-G${refs.totalPOSRow}-G${refs.totalLodgementRow}-(G${refs.openingBalRow}-G${refs.closingBalRow})`, { font: FONT_BOLD, fill: YELLOW_FILL, border: { top: { style: 'thin' }, bottom: { style: 'double' }, right: { style: 'medium' } }, alignment: { horizontal: 'center' } })
  row++
...
function writeLodgement(wb, report, stationName, startDate, endDate, refs, logoId) {
  ...
  // Track which displayBanks indices count as POS (POS + transfers — excel was designed without transfers)
  const posBankColIndices = []
  for (let i = 0; i < displayBanks.length; i++) {
    posBankColIndices.push(i)
  }
...
function writeProductReceived(wb, receipts, tanks, startDate, endDate, refs) {
  ...
  // Group raw receipts into deliveries (one row per truck, not per tank).
  // Compartment fields (firstCompartment etc.) describe the truck, so summing them
  // per tank entry would double-count when a delivery fills multiple tanks.
  const deliveryMap = {}
  ...
  const deliveries = Object.values(deliveryMap).map(({ first, records }) => {
    ...
    // Assign expected to the delivery's fuel type (all tanks in same delivery share the compartment values)
    const fuelTypes = Object.keys(byFuel)
    if (fuelTypes.length === 1) {
      byFuel[fuelTypes[0]].expected = expected
    } else if (fuelTypes.length > 1) {
      // Multi-fuel delivery: assign expected proportionally by actual volume
      const totalActual = fuelTypes.reduce((s, ft) => s + byFuel[ft].actual, 0)
      for (const ft of fuelTypes) {
  ...
  deliveries.sort((a, b) => (a.entryDate || '').localeCompare(b.entryDate || ''))
  ...
  // Borders per reference — each fuel group has specific patterns
  const hdr7 = (align) => ({ font: FONT_BOLD, alignment: { horizontal: align || 'center' } })
  textCell(ws, 7, 2, 'LOADING ', { ...hdr7(), border: BORDER_MED_BOX })
  textCell(ws, 7, 3, 'RECEIPT', { ...hdr7(), border: { top: { style: 'medium' }, left: { style: 'medium' }, right: { style: 'medium' } } })
  // PMS merge D7:G7 — T:medium|B:medium, edges L/R:medium
  textCell(ws, 7, 4, 'PMS', { ...hdr7(), border: { top: { style: 'medium' }, bottom: { style: 'medium' }, left: { style: 'medium' } } })
  ws.mergeCells('D7:G7')
  // AGO merge I7:L7 — T:medium|B:thin, edges L/R:medium
  textCell(ws, 7, 9, 'AGO', { ...hdr7(), border: { top: { style: 'medium' }, bottom: { style: 'thin' }, left: { style: 'medium' } } })
  ws.mergeCells('I7:L7')
  // DPK merge N7:P7 — T:medium|B:medium, edges L/R:medium
  textCell(ws, 7, 14, 'DPK', { ...hdr7(), border: { top: { style: 'medium' }, bottom: { style: 'medium' }, left: { style: 'medium' } } })
  ws.mergeCells('N7:P7')

  // Row 8: Sub-headers — B:medium based, specific L/R per cell
  textCell(ws, 8, 2, 'DATE', { ...hdr7(), border: { bottom: { style: 'medium' }, left: { style: 'medium' }, right: { style: 'medium' } } })
  textCell(ws, 8, 3, 'DATE', { ...hdr7(), border: BORDER_MED_BOX })
  textCell(ws, 8, 4, 'EXPECTED', { ...hdr7(), border: { bottom: { style: 'medium' }, left: { style: 'medium' } } })
  textCell(ws, 8, 6, 'ACTUAL', { ...hdr7(), border: { bottom: { style: 'medium' } } })
  textCell(ws, 8, 9, 'EXPECTED', { ...hdr7(), border: { bottom: { style: 'medium' }, left: { style: 'medium' } } })
  textCell(ws, 8, 11, 'ACTUAL', { ...hdr7(), border: { bottom: { style: 'medium' } } })
  textCell(ws, 8, 14, 'EXPECTED', { ...hdr7(), border: { bottom: { style: 'medium' } } })
  textCell(ws, 8, 16, 'ACTUAL', { ...hdr7(), border: { bottom: { style: 'medium' }, right: { style: 'medium' } } })

  // Data rows starting at row 9
  let row = 9
  for (const del of deliveries) {
    const pmsExp = del.byFuel.PMS?.expected || 0
    const pmsAct = del.byFuel.PMS?.actual || 0
    const agoExp = del.byFuel.AGO?.expected || 0
    const agoAct = del.byFuel.AGO?.actual || 0
    const dpkExp = del.byFuel.DPK?.expected || 0
    const dpkAct = del.byFuel.DPK?.actual || 0

    dateCell(ws, row, 2, del.loadedDate || del.entryDate, dataStyle('center', FONT_10))
    dateCell(ws, row, 3, del.entryDate, dataStyle('center', FONT_10))
    if (pmsExp) numCell(ws, row, 4, pmsExp, dataStyle('center', FONT_10), ACCT_FMT)
    if (pmsAct) numCell(ws, row, 6, pmsAct, dataStyle('center', FONT_10), ACCT_FMT)
    if (agoExp) numCell(ws, row, 9, agoExp, dataStyle('center', FONT_10), ACCT_FMT)
    if (agoAct) numCell(ws, row, 11, agoAct, dataStyle('center', FONT_10), ACCT_FMT)
    if (dpkExp) numCell(ws, row, 14, dpkExp, dataStyle('center', FONT_10), ACCT_FMT)
    if (dpkAct) numCell(ws, row, 16, dpkAct, dataStyle('center', FONT_10), ACCT_FMT)
    row++
  }
  ...
  // Totals row at row 52 — T:medium|B:medium per reference
  const totRow = 52
  refs.productReceivedTotalRow = totRow
  const prTotBorder = { top: { style: 'medium' }, bottom: { style: 'medium' } }
  const prTotStyle = (extraBorder) => ({ font: FONT_BOLD, border: { ...prTotBorder, ...extraBorder }, alignment: { horizontal: 'center' } })

  textCell(ws, totRow, 2, 'TOTAL', { font: FONT_10_BOLD, border: { ...prTotBorder, left: { style: 'medium' } } })
  formulaCell(ws, totRow, 4, 'SUM(D9:D51)', prTotStyle({ left: { style: 'thin' }, right: { style: 'thin' } }), ACCT_FMT)
  formulaCell(ws, totRow, 6, 'SUM(F9:F51)', prTotStyle({ left: { style: 'thin' }, right: { style: 'thin' } }), ACCT_FMT)
  formulaCell(ws, totRow, 9, 'SUM(I9:I51)', prTotStyle({ left: { style: 'thin' }, right: { style: 'thin' } }), ACCT_FMT)
  formulaCell(ws, totRow, 11, 'SUM(K9:K51)', prTotStyle(), ACCT_FMT)
  formulaCell(ws, totRow, 14, 'SUM(N9:N51)', prTotStyle(), ACCT_FMT)
  formulaCell(ws, totRow, 16, 'SUM(P9:P51)', prTotStyle({ right: { style: 'medium' } }), ACCT_FMT)

  // Fill header rows with edge borders
  fillHeaderRow(ws, 7, 2, 16, FONT_BOLD)
  fillHeaderRow(ws, 8, 2, 16, FONT_BOLD)
...
function writeExpenses(wb, expenses) {
  ...
  ws.getColumn(7).width = 2.71

  // Row 4: Title
  textCell(ws, 4, 4, 'EXPENSES FOR THE MONTH', { font: FONT_10_BOLD, border: {}, alignment: { horizontal: 'center' } })

  // Row 6: Headers — specific borders per cell from reference
  const expHdr = (brd, valign) => ({ font: FONT_10_BOLD, border: brd, alignment: { horizontal: 'center', vertical: valign || 'middle' } })
  textCell(ws, 6, 2, 'S/N', expHdr(BORDER_MED_BOX))
  textCell(ws, 6, 3, 'DATE', expHdr({ top: { style: 'medium' }, bottom: { style: 'medium' }, right: { style: 'medium' } }))
  textCell(ws, 6, 4, 'PURPOSE  OF EXPENSES ', expHdr(BORDER_HDR_TB))
  textCell(ws, 6, 5, ' AMOUNT (=N=)', { font: FONT_10_BOLD, border: BORDER_MED_BOX, alignment: { horizontal: 'center', wrapText: true } })
  textCell(ws, 6, 6, 'APPROVED BY:', expHdr(BORDER_MED_BOX, undefined))
  ...
  // Totals row 38 — B:medium per reference (not T:thin|B:double)
  textCell(ws, 38, 3, 'Total', { font: FONT_10_BOLD, border: { bottom: { style: 'medium' } } })
  formulaCell(ws, 38, 5, 'SUM(E7:E37)', { font: FONT_10_BOLD, border: { bottom: { style: 'medium' }, left: { style: 'medium' } } }, ACCT_PAREN)
...
function writeRecordOfStock(wb, report, refs) {
  ...
  // Column widths matching reference — 20 columns
  // PMS: B-F, spacer G, AGO: H-L, spacer M, DPK: N-R, spacer S-T
  ...
  ws.getColumn(19).width = 4.29   // S - spacer
  ws.getColumn(20).width = 9.29   // T - spacer

  // Row 6: Fuel type section headers (merged)
  // PMS/AGO: T:medium only (no B:medium), DPK: T:medium|B:medium, edges L/R:medium
  textCell(ws, 6, 2, 'PMS', { font: FONT_BOLD, alignment: { horizontal: 'center' }, border: { top: { style: 'medium' }, left: { style: 'medium' } } })
  ws.mergeCells('B6:F6')
  textCell(ws, 6, 8, 'AGO', { font: FONT_BOLD, alignment: { horizontal: 'center' }, border: { top: { style: 'medium' }, left: { style: 'medium' } } })
  ws.mergeCells('H6:L6')
  textCell(ws, 6, 14, 'DPK', { font: FONT_BOLD, alignment: { horizontal: 'center' }, border: { top: { style: 'medium' }, bottom: { style: 'medium' }, left: { style: 'medium' } } })
  ws.mergeCells('N6:R6')

  // Row 7: Column headers — T:medium|B:medium with specific L/R per position
  const sectionHeaders = ['Date', 'Opening Stock', 'Actual Producted Supplied', 'Quantity sold', 'Closing Stock']
  // Per-position borders: [Date, Opening, Supply, QtySold, Closing]
  const hdrBorders = [
    { top: { style: 'medium' }, bottom: { style: 'medium' }, left: { style: 'medium' }, right: { style: 'thin' } },  // Date
    { top: { style: 'medium' }, bottom: { style: 'medium' }, left: { style: 'thin' }, right: { style: 'thin' } },    // Opening
    { top: { style: 'medium' }, bottom: { style: 'medium' }, left: { style: 'thin' } },                              // Supply (no R)
    { top: { style: 'medium' }, bottom: { style: 'medium' }, left: { style: 'thin' }, right: { style: 'thin' } },    // Qty Sold
    { top: { style: 'medium' }, bottom: { style: 'medium' }, right: { style: 'medium' } },                           // Closing (no L)
  ]
  // PMS (B-F), AGO (H-L), DPK (N-R)
  for (const baseCol of [2, 8, 14]) {
    for (let i = 0; i < sectionHeaders.length; i++) {
      textCell(ws, 7, baseCol + i, sectionHeaders[i], { font: FONT_10_BOLD, border: hdrBorders[i], alignment: { horizontal: 'center' } })
    }
  }
  ...
  for (const ft of fuelTypes) {
    ...
    // PMS uses ACCT_FMT for opening/closing; AGO/DPK use ACCT_FMT_2
    const valFmt = ft === 'PMS' ? ACCT_FMT : ACCT_FMT_2

    for (let i = 0; i < visibleRows.length; i++) {
      const r = dataStartRow + i
      const d = visibleRows[i]

      dateCell(ws, r, cols.dateCol, d.date, dataStyle('right', FONT_10))
      numCell(ws, r, cols.openCol, d.opening, dataStyle('center', FONT_10), valFmt)
      if (d.supply) numCell(ws, r, cols.supplyCol, d.supply, dataStyle('center', FONT_10), ACCT_FMT_2)

      // Qty Sold = Opening + Supply - Closing (yellow formula) — ACCT_FMT for all fuels
      const oc = colLetter(cols.openCol)
      const sc = colLetter(cols.supplyCol)
      const cc = colLetter(cols.closeCol)
      formulaYellow(ws, r, cols.qtyCol, `${oc}${r}+${sc}${r}-${cc}${r}`, dataStyleYellow('center', FONT_10), ACCT_FMT)

      numCell(ws, r, cols.closeCol, d.closing, dataStyle('center', FONT_10), valFmt)
    }
  }
  ...
  // Row 38: Last data row with closing stock repeated + B:medium on qty sold
  const r38 = 38
  const qtyBorderR38 = { ...BORDER_THIN, bottom: { style: 'medium' } }
  for (const ft of fuelTypes) {
    const data = stockPosition[ft]
    if (!data) continue
    const cols = fuelCols[ft]
    ...
    const t = data.totals
    const valFmt = ft === 'PMS' ? ACCT_FMT : ACCT_FMT_2
    numCell(ws, r38, cols.openCol, t.closing, dataStyle('center', FONT_10), valFmt)
    const oc = colLetter(cols.openCol)
    const sc = colLetter(cols.supplyCol)
    const cc = colLetter(cols.closeCol)
    formulaYellow(ws, r38, cols.qtyCol, `${oc}${r38}+${sc}${r38}-${cc}${r38}`,
      { font: FONT_10, fill: YELLOW_FILL, border: qtyBorderR38, alignment: { horizontal: 'center' } }, ACCT_FMT)
    numCell(ws, r38, cols.closeCol, t.closing, dataStyle('center', FONT_10), valFmt)
  }

  // Row 40: Closing stock formula — IF(C38>0, F38, F37) — yellow fill, center, no borders
  for (const ft of fuelTypes) {
    const cols = fuelCols[ft]
    if (!cols) continue
    const oc = colLetter(cols.openCol)
    const cc = colLetter(cols.closeCol)
    formulaYellow(ws, 40, cols.closeCol, `IF(${oc}38>0,${cc}38,${cc}37)`,
      { font: FONT, fill: YELLOW_FILL, alignment: { horizontal: 'center' } })
  }

  // Fill header rows with edge borders for each section
  // PMS (B-F), AGO (H-L), DPK (N-R)
  fillHeaderRow(ws, 6, 2, 6, FONT_BOLD)
  fillHeaderRow(ws, 6, 8, 12, FONT_BOLD)
  fillHeaderRow(ws, 6, 14, 18, FONT_BOLD)

  // Fill data grids per section (rows 8-38 only; row 40 is standalone)
  fillGrid(ws, 8, 38, 2, 6, FONT_10)
  fillGrid(ws, 8, 38, 8, 12, FONT_10)
  fillGrid(ws, 8, 38, 14, 18, FONT_10)
...
function writeCustomersLedger(wb, customers) {
  ...
  // Row 1: Title (Arial 14 bold, merged A1:J1) — L:thin per reference
  textCell(ws, 1, 1, 'CORPORATE CUSTOMERS STATEMENT FOR THE MONTH', { font: ARIAL_TITLE, border: { left: { style: 'thin' } }, alignment: { horizontal: 'center', wrapText: true } })
  ws.mergeCells('A1:J1')
  ws.getRow(1).height = 19.15

  // Row 2: Headers (Arial 11 bold) — specific borders per cell from reference
  const clHdr = (brd, align) => ({ font: ARIAL_BOLD, border: brd, alignment: { horizontal: align || 'center', vertical: 'middle', wrapText: true } })
  textCell(ws, 2, 1, 'S/N', clHdr({ top: { style: 'medium' }, bottom: { style: 'medium' }, left: { style: 'medium' }, right: { style: 'thin' } }))
  textCell(ws, 2, 2, "CUSTOMER'S NAME", { font: ARIAL_BOLD, border: { top: { style: 'medium' }, bottom: { style: 'medium' }, left: { style: 'thin' }, right: { style: 'thin' } }, alignment: { horizontal: 'center', wrapText: true } })
  textCell(ws, 2, 3, 'OPENING BALANCE', { font: ARIAL_BOLD, border: { top: { style: 'medium' }, bottom: { style: 'medium' }, left: { style: 'thin' } }, alignment: { wrapText: true } })
  textCell(ws, 2, 5, 'LODGEMENT DATE ', { font: ARIAL_BOLD, border: { top: { style: 'medium' }, bottom: { style: 'medium' }, right: { style: 'thin' } }, alignment: { horizontal: 'center', wrapText: true } })
  textCell(ws, 2, 6, 'BANK', clHdr({ top: { style: 'medium' }, bottom: { style: 'medium' }, left: { style: 'thin' }, right: { style: 'thin' } }))
  textCell(ws, 2, 7, 'AMOUNT LODGED', { font: ARIAL_BOLD, border: { top: { style: 'medium' }, bottom: { style: 'medium' }, left: { style: 'thin' }, right: { style: 'medium' } }, numFmt: ACCT_PAREN, alignment: { horizontal: 'center', wrapText: true } })
  textCell(ws, 2, 9, 'VALUE OF PRODUCT SOLD', { font: ARIAL_BOLD, border: BORDER_MED_BOX, numFmt: ACCT_FMT, alignment: { horizontal: 'center', wrapText: true } })
  textCell(ws, 2, 10, 'CLOSING BALANCE ', { font: ARIAL_BOLD, border: BORDER_MED_BOX, numFmt: ACCT_FMT, alignment: { horizontal: 'center', vertical: 'middle', wrapText: true } })
  ...
  // J column formula border: B:thin|R:medium per reference
  const jBorder = { bottom: { style: 'thin' }, right: { style: 'medium' } }
  const jStyle = { font: ARIAL_FONT, fill: YELLOW_FILL, border: jBorder, alignment: { horizontal: 'center' } }

  // Row 3: J3 = C3+G3-I3 (first formula is unique)
  formulaYellow(ws, 3, 10, 'C3+G3-I3', jStyle)
  textCell(ws, 3, 1, 1, dataSty)

  // Row 4: J4 has own formula (start of shared range)
  formulaYellow(ws, 4, 10, 'C4+G4-I4', jStyle)

  // Rows 5-42: shared formula
  for (let r = 5; r <= 42; r++) {
    formulaYellow(ws, r, 10, `C${r}+G${r}-I${r}`, jStyle)
  }
  ...
  // Row 43: Totals — T:medium|B:medium per reference
  const clTotBrd = { top: { style: 'medium' }, bottom: { style: 'medium' }, left: { style: 'thin' } }
  textCell(ws, 43, 2, 'TOTAL', { font: ARIAL_BOLD, border: { top: { style: 'medium' }, bottom: { style: 'medium' }, left: { style: 'thin' }, right: { style: 'thin' } }, numFmt: ACCT_PAREN_2 })
  formulaYellow(ws, 43, 3, 'SUM(C3:C42)', { font: ARIAL_BOLD, fill: YELLOW_FILL, border: clTotBrd, alignment: { horizontal: 'center' } }, ACCT_PAREN)
  formulaYellow(ws, 43, 7, 'SUM(G3:G42)', { font: ARIAL_BOLD, fill: YELLOW_FILL, border: clTotBrd, alignment: { horizontal: 'center' } }, ACCT_PAREN)
  formulaYellow(ws, 43, 9, 'SUM(I3:I42)', { font: ARIAL_BOLD, fill: YELLOW_FILL, border: clTotBrd, alignment: { horizontal: 'center' } })
  formulaYellow(ws, 43, 10, 'SUM(J3:J42)', { font: ARIAL_BOLD, fill: YELLOW_FILL, border: { top: { style: 'medium' }, bottom: { style: 'medium' }, right: { style: 'medium' } }, alignment: { horizontal: 'center' } })
...
function writeLubricants(wb, lubeSales, lubeStock, lubeProducts, stationName, startDate, endDate) {
  ...
  // Column widths matching reference (15 columns A-O)
  ...
  ws.getColumn(12).width = 8.71  // L
  ws.getColumn(13).width = 8.71  // M
  ws.getColumn(14).width = 8.71  // N
  ws.getColumn(15).width = 8.71  // O

  // Row 3: Title (Calibri 12 bold, merged B3:J3) — edges L/R:medium
  textCell(ws, 3, 2, `CASTRO OIL AS AT ${fmtDateDisplay(endDate)} - ${stationName || ''}`, { font: CAL_12_BOLD, border: { top: { style: 'medium' }, bottom: { style: 'medium' }, left: { style: 'medium' } }, alignment: { horizontal: 'center' } })
  ws.mergeCells('B3:J3')
  ws.getRow(3).height = 15.75

  // Row 4: Headers — B:thin based (not T:medium|B:medium), Date (c11) has BORDER_MED_BOX
  const lubeHdr = (extraBrd) => ({ font: CAL_BOLD, border: { bottom: { style: 'thin' }, left: { style: 'thin' }, right: { style: 'thin' }, ...extraBrd }, alignment: { horizontal: 'center', wrapText: true } })
  textCell(ws, 4, 2, 'S/N', lubeHdr({ left: { style: 'medium' } }))
  textCell(ws, 4, 3, 'ITEM', lubeHdr())
  textCell(ws, 4, 4, 'Litre', lubeHdr())
  textCell(ws, 4, 5, 'OPENING STOCK ', lubeHdr())
  textCell(ws, 4, 6, 'PURCHASE', lubeHdr())
  textCell(ws, 4, 7, 'SALES (QUANTITY)', { font: CAL_BOLD, fill: YELLOW_FILL, border: { bottom: { style: 'thin' }, left: { style: 'thin' }, right: { style: 'thin' } }, alignment: { horizontal: 'center', wrapText: true } })
  textCell(ws, 4, 8, 'CLOSING STOCK', lubeHdr())
  textCell(ws, 4, 9, 'PRICE (N)', lubeHdr())
  textCell(ws, 4, 10, 'AMOUNT', { font: CAL_BOLD, fill: YELLOW_FILL, border: { bottom: { style: 'thin' }, left: { style: 'thin' }, right: { style: 'medium' } }, numFmt: ACCT_FMT_2, alignment: { horizontal: 'center', wrapText: true } })
  textCell(ws, 4, 11, 'Date', { font: CAL_BOLD, border: BORDER_MED_BOX, alignment: { horizontal: 'center', wrapText: true } })
  ...
  for (let idx = 0; idx < products.length; idx++) {
    ...
    textCell(ws, row, 3, prod.product_name || '', dataStyle('left', CAL_BOLD))
    numCell(ws, row, 4, prod.size || 0, dataStyle('center', CAL_FONT))
    ...
    numCell(ws, row, 9, unitPrice, dataStyle('center', CAL_FONT), ACCT_PAREN)
  ...
  // Row 30: Totals — T:medium|B:medium per reference
  const totRow = 30
  const lubeTotBrd = { top: { style: 'medium' }, bottom: { style: 'medium' }, left: { style: 'thin' }, right: { style: 'thin' } }
  formulaCell(ws, totRow, 5, `SUM(E${dataStartRow}:E29)`, { font: CAL_BOLD, border: lubeTotBrd, alignment: { horizontal: 'center' } })
  formulaCell(ws, totRow, 6, `SUM(F${dataStartRow}:F29)`, { font: CAL_BOLD, border: lubeTotBrd, alignment: { horizontal: 'center' } })
  formulaCell(ws, totRow, 7, `SUM(G${dataStartRow}:G29)`, { font: CAL_BOLD, border: lubeTotBrd, alignment: { horizontal: 'center' } })
  formulaCell(ws, totRow, 8, `SUM(H${dataStartRow}:H29)`, { font: { name: 'Calibri', size: 12, color: { theme: 1 } }, border: lubeTotBrd, alignment: { horizontal: 'center' } })
  textCell(ws, totRow, 9, 'TOTAL', { font: CAL_BOLD, border: BORDER_MED_BOX, alignment: { horizontal: 'center' } })
  formulaYellow(ws, totRow, 10, `SUM(J${dataStartRow}:J29)`, { font: CAL_BOLD, fill: YELLOW_FILL, border: { top: { style: 'medium' }, right: { style: 'medium' } }, alignment: { horizontal: 'center' } }, ACCT_PAREN_2)
  ws.getRow(totRow).height = 15.75

  // Rows 31-35: LODGED rows — bold, T:medium|B:medium|L:medium on I
  for (let r = 31; r <= 35; r++) {
    textCell(ws, r, 9, 'LODGED', { font: CAL_BOLD, border: { top: { style: 'medium' }, bottom: { style: 'medium' }, left: { style: 'medium' } } })
  }

  // Row 36: Balance = Total Amount - Sum of Lodged — Calibri 14 bold, BORDER_THIN, ACCT_PAREN_2
  formulaYellow(ws, 36, 10, `J${totRow}-SUM(J31:J35)`, { font: CAL_TITLE, fill: YELLOW_FILL, border: BORDER_THIN, alignment: { horizontal: 'center' } }, ACCT_PAREN_2)
  ws.getRow(36).height = 18.75

  // Fill data grid with borders on all empty cells
  fillGrid(ws, 5, 29, 2, 10, CAL_FONT)
}
