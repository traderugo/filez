...
const LODGEMENT_TYPE_ORDER = ['pos', 'bank_deposit', 'cash', 'other']
...
/**
 * Sort banks by lodgement_type order then sort_order.
 */
 ...
 * @returns {Object} { dates: [{ date, bankRows, totalPOS, totalDeposits, totalCash, totalAll, hasEntry, entryCount }], bankIds }
 */
function calculateDateRangeLodgements(allLodgements, banks, startDate, endDate) {
  ...
  const dates = dateStrings.map(date => {
    ...
    const totalAll = totalPOS + totalDeposits + totalCash + totalOther
