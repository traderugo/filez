...
function buildConsumptionReport({ sales, consumption, nozzles, fuelTypes, customerMap, defaultCustomers, startDate, endDate }) {
  ...
  // Aggregate nozzle readings consumption/pour-back per date per fuel type
  // (entered via the daily sales form rather than the consumption form)
  const STATION_ID = '__station__'
  const nozzlesByFuelCR = {}
  for (const ft of fuelTypes) nozzlesByFuelCR[ft] = nozzles.filter(n => n.fuel_type === ft)

  const nozzleConsumptionByDateFuel = {} // `${date}|${ft}` → { consumed, pourBack }
  for (const entry of rangeEntries) {
    for (const ft of fuelTypes) {
      let consumed = 0, pourBack = 0
      for (const n of nozzlesByFuelCR[ft]) {
        const r = (entry.nozzleReadings || []).find(r => r.pump_id === n.id)
        if (!r) continue
        consumed += Number(r.consumption || 0)
        pourBack += Number(r.pour_back || 0)
      }
      if (consumed || pourBack) {
        const key = `${entry.entryDate}|${ft}`
        if (!nozzleConsumptionByDateFuel[key]) nozzleConsumptionByDateFuel[key] = { consumed: 0, pourBack: 0 }
        nozzleConsumptionByDateFuel[key].consumed += consumed
        nozzleConsumptionByDateFuel[key].pourBack += pourBack
      }
    }
  }
  ...
  for (const ft of fuelTypes) {
    ...
    // Add synthetic Station customer if nozzle readings have consumption for this fuel type
    const hasNozzleConsumed = Object.keys(nozzleConsumptionByDateFuel)
      .some(k => k.endsWith(`|${ft}`) && nozzleConsumptionByDateFuel[k].consumed > 0)
    if (hasNozzleConsumed) customerIds.add(STATION_ID)

    const extendedCustomerMap = { ...customerMap, [STATION_ID]: 'Station' }
    const customers = [...customerIds].map(id => ({ id, name: extendedCustomerMap[id] || 'Unknown' }))
    ...
    const rows = dateStrings.map(date => {
      ...
      // Include nozzle readings consumption for this date
      const nozzleDay = nozzleConsumptionByDateFuel[`${date}|${ft}`]
      if (nozzleDay) {
        if (nozzleDay.consumed) customerQtys[STATION_ID] = (customerQtys[STATION_ID] || 0) + nozzleDay.consumed
        if (nozzleDay.pourBack) pourBack += nozzleDay.pourBack
      }

      return { date, rate, customerQtys, pourBack, hasData: dayEntries.length > 0 || !!nozzleDay }
    })
