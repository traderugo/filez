...
function writeConsumption(wb, report, startDate, endDate, fuelType, sheetName) {
  ...
  // DPK Rate column uses #,##0 format + right alignment; others use General + center
  const rateFmt = fuelType === 'DPK' ? NUM_FMT : 'General'
  const rateAlign = fuelType === 'DPK' ? 'right' : 'center'
  // AGO first customer column (col 4) uses ACCT_FMT; others use General
  const agoFirstCust = fuelType === 'AGO'

  let row = hdrRow + 1
  for (const r of dataRows) {
    dateCell(ws, row, 2, r.date, dataStyle('right', FONT_10))
    numCell(ws, row, 3, r.rate, dataStyle(rateAlign, FONT_10), rateFmt)
    for (let i = 0; i < customers.length; i++) {
      const custFmt = (agoFirstCust && i === 0) ? ACCT_FMT : 'General'
      numCell(ws, row, 4 + i, r.customerQtys[customers[i].id] || 0, dataStyle('center', FONT_10), custFmt)
    }
    numCell(ws, row, 4 + customers.length, r.pourBack || 0, dataStyle('center', FONT_10), 'General')
    row++
  }

  // Fill remaining template rows — per fuel type from reference
  // PMS: 30 data rows (6-35), AGO: 32 data rows (6-37), DPK: 30 data rows (7-36)
  const templateRowCounts = { PMS: 30, AGO: 32, DPK: 30 }
  const templateRows = templateRowCounts[fuelType] || 30
  const written = dataRows.length
  for (let i = written; i < templateRows; i++) {
    row++
  }

  // Totals row — T:medium|B:medium per reference (not T:thin|B:double)
  const totRow = row
  const totBorder = { top: { style: 'medium' }, bottom: { style: 'medium' } }
  const consumeTotStyle = (align) => ({ font: FONT_10_BOLD, border: { ...totBorder, left: { style: 'thin' }, right: { style: 'thin' } }, alignment: { horizontal: align || 'center' } })

  textCell(ws, totRow, 2, 'Total', consumeTotStyle('left'))
  textCell(ws, totRow, 3, '', consumeTotStyle('center'))
  for (let i = 0; i < customers.length; i++) {
    const custTotFmt = (agoFirstCust && i === 0) ? ACCT_FMT : ACCT_FMT_PLAIN
    numCell(ws, totRow, 4 + i, totals.customerTotals[customers[i].id] || 0, consumeTotStyle('center'), custTotFmt)
  }
  numCell(ws, totRow, 4 + customers.length, totals.pourBack || 0, consumeTotStyle('center'), ACCT_FMT_PLAIN)
