        ...
        if (res.ok) {
          ...
          if (item.operation !== 'DELETE') {
            ...
            if (data.entry && TABLE_API_MAP[item.table]) {
              // Update local record with any server-generated fields, preserving local-only fields
          ...
          // Validation error — discard (won't fix itself on retry)
          console.warn(`Sync: dropping invalid item (400)`, item)
          await db.syncQueue.delete(item.queueId)
        } else if (res.status === 401 || res.status === 403) {
          // Auth/subscription error — stop processing, retry after re-login
          console.warn(`Sync: auth error (${res.status}), will retry later`)
          break
        } else if (res.status >= 402 && res.status < 500) {
          // Other client errors — discard
