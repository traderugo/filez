...
function buildFuelMeterRows(fuelType, nozzles, allSorted, prevEntry, dateStrings, entriesByDate) {
  ...
  for (const date of dateStrings) {
    ...
    if (dayEntries.length === 1) {
      ...
      // First, close any accumulated open row from the previous period
      // so it doesn't get merged with today's first entry.
      if (hasOpenRow) {
        emitRow(currentPrice)
      }
      ...
      for (let i = 0; i < dayEntries.length; i++) {
        ...
        // Update lastClosing from this entry's nozzle readings
        for (const n of ftNozzles) {
          const r = (entry.nozzleReadings || []).find(r => r.pump_id === n.id)
          if (r) lastClosing[n.id] = Number(r.closing_meter || 0)
        }
        ...
        if (i < dayEntries.length - 1) {
          // Not the last entry — emit immediately as its own row
          emitRow(entryPrice)
        } else {
          // Last entry — keep open for the next day to accumulate
          currentPrice = entryPrice
          hasOpenRow = true
        }
      }
