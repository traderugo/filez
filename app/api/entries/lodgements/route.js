...
export async function GET(request) {
  try {
    ...
    if (id) {
      const { data } = await supabase.from(TABLE).select('*, users:created_by(name), bank:bank_id(id, bank_name, lodgement_type)').eq('id', id).eq('org_id', user.org_id).is('deleted_at', null).single()
