...
function writeSalesCash(wb, report, stationName, startDate, endDate, refs) {
  ...
  // No merges — label in B overflows naturally across C-F (those cells stay empty).
  // Data rows: thin borders on B-G with medium left on B, medium right on G.
  // Spacer rows: only medium vertical edges on B and G.
  const reconRowBorder = { top: { style: 'thin' }, bottom: { style: 'thin' }, left: { style: 'thin' }, right: { style: 'thin' } }
  const reconRowBorderL = { top: { style: 'thin' }, bottom: { style: 'thin' }, left: { style: 'medium' }, right: { style: 'thin' } }
  const reconRowBorderR = { top: { style: 'thin' }, bottom: { style: 'thin' }, left: { style: 'thin' }, right: { style: 'medium' } }

  const reconRow = (r, text, formula) => {
    // Label in B — text overflows across C-F
    textCell(ws, r, 2, text, { font: FONT_BOLD, border: reconRowBorderL, alignment: { horizontal: 'left' } })
    // C-F: thin borders only (no content, so B text overflows)
    for (let c = 3; c <= 6; c++) {
      ws.getRow(r).getCell(c).border = reconRowBorder
      ws.getRow(r).getCell(c).font = FONT
    }
    // G: yellow value
    formulaYellow(ws, r, 7, formula, { font: FONT_BOLD, fill: YELLOW_FILL, border: reconRowBorderR, alignment: { horizontal: 'center' } })
  }
  const reconSpacer = (r) => {
  ...
  // Header row — medium bottom border, label in B
  ...
  // Overage/Shortage
  reconRow(row, 'Overage/Shortage', `(G${refs.totalLodgementRow}+G${refs.totalPOSRow}+G${refs.openingBalRow}-G${refs.closingBalRow})-G${refs.netExpectedRow}`)
}
