...
function writeSalesCash(wb, report, stationName, startDate, endDate, refs) {
  ...
  for (let ftIdx = 0; ftIdx < fuelTypes.length; ftIdx++) {
    ...
    textCell(ws, row, 2, String(ftIdx + 1), { font: FONT_BOLD, border: BORDER_THIN, alignment: { horizontal: 'center' } })
    textCell(ws, row, 3, `${ft} Sales for the period`, { font: FONT_BOLD, border: BORDER_THIN, alignment: { horizontal: 'center' } })
    for (let c = 4; c <= 7; c++) applyStyle(ws.getRow(row).getCell(c), { border: BORDER_THIN })
    ws.mergeCells(row, 3, row, 7)
    row++

    // Meter reading headers
    textCell(ws, row, 2, 'Meter reading', hdrStyle(FONT_BOLD))
    textCell(ws, row, 3, 'Opening', hdrStyle(FONT_BOLD))
    textCell(ws, row, 4, 'Closing', hdrStyle(FONT_BOLD))
    textCell(ws, row, 5, 'Quantity dispensed', hdrStyle(FONT_BOLD))
