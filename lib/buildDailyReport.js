...
export function buildDailyReport({ sales, receipts, lodgements, consumption, nozzles, tanks, banks, startDate, endDate }) {
  ...
  const dateReports = rangeResult.dates.map(({ date, entries: helperEntries, hasEntry, entryCount }) => {
    ...
    // Override dayFuelTotals with db.consumption values ONLY when the
    // consumption form has entries for that fuel type. Otherwise keep the
    // nozzle-level consumption already computed above (lines 101-128).
    const overriddenFuels = new Set()
    for (const ft of fuelTypes) {
      ...
      if (entryConsumed > 0 || entryPourBack > 0) {
        ...
        overriddenFuels.add(ft)
      }
    }

    // Propagate db.consumption override to per-entry subtotals only for
    // fuel types where the override was applied. Nozzle-level consumption
    // is already correct in entryFuelTotals for non-overridden fuel types.
    if (entryGroups.length === 1) {
      for (const ft of overriddenFuels) {
        const eft = entryGroups[0].fuelTotals[ft]
        if (eft) {
          eft.consumed = dayFuelTotals[ft].consumed
          eft.pourBack = dayFuelTotals[ft].pourBack
          eft.actual = dayFuelTotals[ft].actual
          eft.amount = eft.actual * eft.price
        }
      }
    } else if (entryGroups.length > 1) {
      for (const ft of overriddenFuels) {
        const total = dayFuelTotals[ft].dispensed
        if (!total) continue
        for (const eg of entryGroups) {
          const eft = eg.fuelTotals[ft]
          if (!eft || !eft.dispensed) continue
          const ratio = eft.dispensed / total
          eft.consumed = dayFuelTotals[ft].consumed * ratio
          eft.pourBack = dayFuelTotals[ft].pourBack * ratio
          eft.actual = eft.dispensed - eft.consumed - eft.pourBack
          eft.amount = eft.actual * eft.price
        }
      }
    }

    // Recompute day-level amount from per-entry totals (each uses its own price)
    for (const ft of overriddenFuels) {
      dayFuelTotals[ft].amount = entryGroups.reduce((sum, eg) => sum + (eg.fuelTotals[ft]?.amount || 0), 0)
    }
