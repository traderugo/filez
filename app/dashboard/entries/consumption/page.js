...
export default function ConsumptionFormPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editId) {
        const existing = await consumptionRepo.getById(editId)
  ...
  const handleDelete = async () => {
    ...
    await consumptionRepo.remove(editId, orgId)
    router.push(`/dashboard/entries/consumption/list?${qs}`)
  }
  ...
  return (
    <div className="max-w-3xl px-4 sm:px-8 py-8">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-xl font-bold text-gray-900">{editId ? 'Edit Entry' : 'New Consumption Entry'}</h1>
        ...
        <div className="border border-gray-300 divide-y divide-gray-300">
          ...
          <div className="grid grid-cols-2 divide-x divide-gray-300">
            <div>
              ...
              <input type="number" value={quantity} onChange={(e) => setQuantity(e.target.value)} step="0.01" min="0" placeholder="0.00" className="w-full px-2 py-1.5 text-sm bg-transparent focus:outline-none focus:bg-blue-50" />
            </div>
            <div>
              <label className="block text-[10px] text-gray-400 px-2 pt-1 uppercase tracking-wide">Fuel Type</label>
              <select value={fuelType} onChange={(e) => setFuelType(e.target.value)} className="w-full px-2 py-1.5 text-sm bg-transparent focus:outline-none focus:bg-blue-50">
