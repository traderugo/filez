...
function buildSalesCashPosition({ sales, lodgements, consumption, nozzles, banks, fuelTypes, customerMap, startDate, endDate }) {
  ...
  for (const ft of fuelTypes) {
    ...
    fuelData[ft] = buildFuelMeterRows(ft, ftNozzles, pumpIds, rangeEntries, prevEntry)
  }
 ...
 *
 * Groups entries into consecutive price periods.
 * Each time the price changes, a new row is started.
 * The same price appearing at different times produces separate rows.
 *
 ...
 */
function buildFuelMeterRows(fuelType, ftNozzles, pumpIds, rangeEntries, prevEntry) {
  const rows = []
  if (!rangeEntries.length) return { rows }

  // Opening meters: last closing from prevEntry, or nozzle initial_reading
  const periodOpening = {}
  for (const n of ftNozzles) {
    const r = prevEntry && (prevEntry.nozzleReadings || []).find(r => r.pump_id === n.id)
    periodOpening[n.id] = r ? Number(r.closing_meter || 0) : Number(n.initial_reading || 0)
  }
  const lastClosing = { ...periodOpening }

  // Price per date: if an entry on a given day has no price, use another entry on the same day
  const priceByDate = {}
  for (const e of rangeEntries) {
    const p = Number(e.prices?.[fuelType] || 0)
    if (p) priceByDate[e.entryDate] = p
  }

  // Resolve price for an entry (own price → same-day price → carry-forward)
  let lastPrice = Number(prevEntry?.prices?.[fuelType] || 0)
  // If prevEntry has no price, seed from first entry in range so we don't start at price=0
  if (!lastPrice) {
    for (const e of rangeEntries) {
      const p = Number(e.prices?.[fuelType] || 0) || priceByDate[e.entryDate] || 0
      if (p) { lastPrice = p; break }
    }
  }
  const priceOf = e => {
    const p = Number(e.prices?.[fuelType] || 0) || priceByDate[e.entryDate] || 0
  ...
  function emitRow() {
    ...
    const pumps = ftNozzles.map(n => {
      const opening = periodOpening[n.id]
      const closing = lastClosing[n.id]
      return { label: `${fuelType} ${n.pump_number}`, pumpId: n.id, opening, closing, dispensed: closing - opening }
    })
    const dispensed = pumps.reduce((s, p) => s + p.dispensed, 0)
    rows.push({ pumps, dispensed, price: currentPrice, amount: dispensed * currentPrice })
    // Former price's closing becomes the next price's opening
    for (const pid of pumpIds) periodOpening[pid] = lastClosing[pid]
    hasReadings = false
  }

  for (const entry of rangeEntries) {
    // Check price change BEFORE processing readings: the entry that records a new price
    // has its meter readings in the NEW period. The old period closes at the last reading
    // recorded BEFORE this entry — former closing becomes next price's opening.
    ...
    for (const pid of pumpIds) {
      const r = (entry.nozzleReadings || []).find(r => r.pump_id === pid)
      if (r) { lastClosing[pid] = Number(r.closing_meter || 0); hasReadings = true }
    }
  }
